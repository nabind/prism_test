...
$(document).ready(function() {
	...
	$(".reportButton").live("click", function(event) {
		...
		if(passSubtest) {
			...
			$(obj).attr('src','openReportHtml.do?reportUrl='+dataURL);
...
var newTab = true;
// Add a tab
function addReportTab(reportUrl, reportId, reportName, assessmentId, isHome)
{	
	blockUI();
	newTab = true;
	// New tab id
	...
	if(newTab) {
		...
		if(!canAddNewTab(tabId)) {
			...
			if(!isHome) { // for home page report URL is present we need to register that only
				...
				getmodel(reportUrl, reportId, reportName, tabId, currentTabCount, assessmentId, tabsAdded);
			}
			tabsAdded = tabsAdded + 1;
			currentTabCount = currentTabCount + 1;
...
function getmodel(reportUrl, reportId, reportName, tabId, tabCount, assessmentId, currentTabNumber) {
	var dataURL = 'path=report/report&reportUrl='+reportUrl+'&reportName='+reportName+'&reportId='+reportId+'&tabCount='+tabCount+'&assessmentId='+assessmentId+'&currentTabNumber='+currentTabNumber+'&studentId='+$(".studentIdForTab").val();
	$.ajax({
		type : "GET",
...
function download(count, reportUrl, assessmentId, type) {
	...
	if(type == 'xls') {
		window.open('download.do'+'?printerFriendly=N&type='+type+'&reportUrl='+reportUrl+'&assessmentId='+assessmentId+'&token=0&'+dataUrl);
	} else {
