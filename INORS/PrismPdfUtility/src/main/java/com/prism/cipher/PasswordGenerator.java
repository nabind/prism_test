...
/**
 * This class is used to generate random string baased on business rule The
 * string can be used for password or salt generation
 * 
 * @author Amit Dhara
 *
 */
public class PasswordGenerator {
	 ...
	 */
	protected static char[] goodSmallChar = { 'a', 'b', 'c', 'd', 'e', 'f', 'h', 'j', 'k', 'm', 'n', 'p', 'r', 's', 't', 'w', 'x', 'y', 'z', };
	protected static char[] goodCapChar = { 'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'J', 'K', 'L', 'M', 'N', 'P', 'Q', 'R', 'S', 'T', 'W', 'X', 'Y', 'Z', };
	protected static char[] goodSpecialChar = { '#', '$', '@', };
	protected static char[] goodDigit = { '2', '3', '4', '5', '6', '7', '8', '9', };
	...
	/**
	 * Generate random text with capital letter at starting and ends with a
	 * digit
	 * 
	 * @return
	 */
	...
	/**
	 * Generate random text without any business validation This text can be
	 * used as a salt
	 * 
	 * @return
	 */
	...
	public static List<OrgTO> populateWithPassword(List<OrgTO> allTenant) {
		for (OrgTO tenant : allTenant) {
			for (UserTO user : tenant.getUsers()) {
				String nextPwd = getNext();
				String salt = getNextSalt();
				user.setPassword(nextPwd);
				user.setSalt(salt);
				user.setEncPassword(SaltedPasswordEncoder.encryptPassword(nextPwd, CustomStringUtil.appendString(user.getUserName().toLowerCase(), salt)));
			}
		}
		return allTenant;
	}

	public static List<OrgTO> populateTeachersWithPassword(List<OrgTO> allTenant) {
		for (OrgTO tenant : allTenant) {
			String nextPwd = getNext();
			String salt = getNextSalt();
			tenant.setPassword(nextPwd);
			tenant.setSalt(salt);
			tenant.setEncPassword(SaltedPasswordEncoder.encryptPassword(nextPwd, tenant.getUserName().toLowerCase() + salt));
		}
		return allTenant;
	}
