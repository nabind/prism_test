...
public class PasswordGenerator {
	...
	public static List<OrgTO> populateWithPassword(List<OrgTO> allTenant) {
		for (OrgTO tenant : allTenant) {
			for (UserTO user : tenant.getUsers()) {
				...
				user.setSalt(salt);
				user.setEncPassword(SaltedPasswordEncoder.encryptPassword(nextPwd, 
						CustomStringUtil.appendString(user.getUserName().toLowerCase(),salt)));
			}
		}
		return allTenant;
	}
	...
	public static List<OrgTO> populateTeachersWithPassword(List<OrgTO> allTenant) {
		for (OrgTO tenant : allTenant) {
			...
			tenant.setSalt(salt);
			tenant.setEncPassword(SaltedPasswordEncoder.encryptPassword(nextPwd, 
					tenant.getUserName().toLowerCase()+salt));
		}
		return allTenant;
	}
