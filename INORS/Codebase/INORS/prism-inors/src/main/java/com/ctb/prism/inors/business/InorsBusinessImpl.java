...
public class InorsBusinessImpl implements IInorsBusiness {
	...
	private static final IAppLogger logger = 
		LogFactory.getLoggerInstance(InorsBusinessImpl.class.getName());
			...
			for(String node : allNodes) {
				...
				if(node.indexOf("|") != -1) {
					...
					if("-1".equals(node.substring(node.indexOf("|")+1, node.length()))) {
						...
						// get all students
						students = adminDAO.getAllStudents(bulkDownloadTO.getTestAdministration(), nodeId, 
								bulkDownloadTO.getCustomerId(), bulkDownloadTO.getGrade());
		...
		for(String node : allNodes) {
			...
			if(node.indexOf("|") != -1) {
				...
				if("-1".equals(node.substring(node.indexOf("|")+1, node.length()))) {
					...
					// get all students
					students = adminDAO.getAllStudents(jobTO.getTestAdministration(), nodeId, 
							jobTO.getCustomerId(), jobTO.getGrade());
					studentCount+= (students == null)? 0 : students.size();
					if(students != null) {
						for(ObjectValueTO obj : students) {
							studentIds.append(obj.getValue()).append(",");
	...
	private void batchPDFDownload(String jobId, BulkDownloadTO jobTO) {
		...
		try {
			for(String studentId : students) {
				...
				String studentFileName = CustomStringUtil.appendString(
						folderLoc, jobTO.getFileName(), studentId, Utils.getDateTime(), ".pdf");
	 ...
	 * @param log
	 */
	private void procesICFiles(BulkDownloadTO jobTO, String folderLoc, String[] students, 
			Map<String, String> isrFileMap, StringBuffer log) {
		OutputStream fos = null;
		InputStream is = null;
		try {
			List<String> isrFiles = new ArrayList<String>();
			...
			if(IApplicationConstants.DOWNLOAD_TYPE_MERGED.equals(jobTO.getRequestType())) {
				...
				for(String studentId : students) {
					...
					archieveFileNames.add(CustomStringUtil.appendString("1-", 
							isrFile.substring(isrFile.lastIndexOf(File.separator) + 1)));
				}
			}
	 ...
	 * @param log
	 */
	private void procesFiles(BulkDownloadTO jobTO, String folderLoc, String[] students, 
			Map<String, String> isrFileMap, StringBuffer log) {
		OutputStream fos = null;
		InputStream is = null;
		try {
			...
			if(IApplicationConstants.DOWNLOAD_TYPE_MERGED.equals(jobTO.getRequestType())) {
				...
				for(String studentId : students) {
					...
					// add image print files
					if("3".equals(jobTO.getGroupFile())) {
						archieveFileNames.add(CustomStringUtil.appendString("1a-", 
								isrFile.substring(isrFile.lastIndexOf(File.separator) + 1)));
						// TODO
						/** GET CODE for IMAGE PRINT */
