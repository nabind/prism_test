...
public class StudentDataUtil {
	...
	public static byte[] createDelimitedByteArray(final List<StudentDataTO> students, final char delimiter) {
		...
		StreamBuilder builder = new StreamBuilder("StudentData").format("delimited").parser(new DelimitedParserBuilder(delimiter)).addRecord(StudentDataTO.class);
	...
	public static byte[] createCSVByteArray(final List<StudentDataTO> students) {
		StreamFactory factory = StreamFactory.newInstance();
		StreamBuilder builder = new StreamBuilder("StudentData").format("csv").parser(new CsvParserBuilder()).addRecord(StudentDataTO.class);
		factory.define(builder);
		StringWriter stringWriter = new StringWriter();
		BeanWriter out = factory.createWriter("StudentData", stringWriter);
		try {
	...
	public static void createFixedLengthFile(final List<StudentDataTO> students, final String filePath) {
		StreamFactory factory = StreamFactory.newInstance();
		StreamBuilder builder = new StreamBuilder("StudentData").format("fixedlength").parser(new FixedLengthParserBuilder()).addRecord(StudentDataTO.class);
		factory.define(builder);
	...
	public static byte[] createXMLByteArray(final List<StudentDataTO> students) {
		StreamFactory factory = StreamFactory.newInstance();
		StreamBuilder builder = new StreamBuilder("StudentData").format("xml").parser(new XmlParserBuilder()).addRecord(StudentDataTO.class);
		factory.define(builder);
		StringWriter stringWriter = new StringWriter();
		BeanWriter out = factory.createWriter("StudentData", stringWriter);
		try {
	...
	/**
	 * Used to add leading space to a String input. Additionally wraps the String with double quote.
	 * 
	 * @param limit
	 * @param str
	 * @return
	...
	/**
	 * Used to add leading character to a String input. Additionally wraps the String with double quote.
	 * 
	 * @param limit
	 * @param strObject
	 * @param c
	...
	/**
	 * Returns a List of mock StudentDataTO objects. This method is for testing purpose only.
	 * 
	 * @return
	 */
