					...
					<c:forEach var="student" items="${studentList}">
						<tr id="">
						    ...
						    <td aria-label="Text: activate to sort column ascending" style="width: 25px; text-align: center;" colspan="1" rowspan="1" aria-controls="sorting-advanced" tabindex="0" role="columnheader" class="sorting vertical-center" scope="col">
								...
								<c:choose>
									<c:when test="${(not empty student.isr) && (student.isr != ' ') && (student.isrFlag == 'Y')}"><a class="button compact icon-read green glossy with-tooltip" target="_blank" title="Individual Student Report" href="downloadZippedPdf.do?fileName=${student.isr}&fileType=Individual_Student_Report"></a></c:when>
									<c:otherwise><a class="button compact icon-forbidden red disabled with-tooltip" title="No ISR Available" href="#"></a></c:otherwise>
								</c:choose>
								<c:choose>
									<c:when test="${(not empty student.ip) && (student.ip != ' ') && (student.ipFlag == 'Y')}"><a class="button compact icon-pages orange glossy with-tooltip" target="_blank" title="Image Print" href="downloadZippedPdf.do?fileName=${student.ip}&fileType=Image_Print"></a></c:when>
