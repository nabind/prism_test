...
<div class="content-panel" style="padding-left: 0px; padding-right: 10px; border: none">
	<form:form method="POST" id="rescoreRequestForm" modelAttribute="rescoreRequestForm">
			...
			<table aria-describedby="sorting-advanced_info" class="table responsive-table responsive-table-on dataTable" id="studentTableRRF">
				<thead>
					<tr role="row">
						...
						<th aria-label="Text: activate to sort column ascending" style="width: 130px;" colspan="1" rowspan="1" aria-controls="sorting-advanced" tabindex="0" role="columnheader" class="" scope="col">ELA<br />Session 2</th>
						<th aria-label="Text: activate to sort column ascending" style="width: 130px;" colspan="1" rowspan="1" aria-controls="sorting-advanced" tabindex="0" role="columnheader" class="" scope="col">ELA<br />Session 3</th>
						    	...
						    	<c:choose>
						    		...
						    		<c:otherwise>
						    			<input type="text" 
											...
											studentBioId="${rescoreRequestStudentTO.studentBioId}" 
						    				id="rescoreDate_${rescoreRequestStudentTO.studentBioId}" 
						    				value="${rescoreRequestStudentTO.requestedDate}" disabled="disabled"/> 
						    		</c:otherwise>
						    	</c:choose>
						    </td>
		...
		<c:when test="${not empty dataloadMessage}">
		<!-- Start Disable All Links -->
		<div class="columns accordion disabled" style="text-align: center; border: none;">
			<span class="button blue-gradient glossy disabled">Review </span>
		</div>
		<!-- End Disable All Links -->
		</c:when>
		<c:otherwise>
				...
				<tbody aria-relevant="all" aria-live="polite" role="alert" id="studentListRRF_2">
					<c:forEach var="rescoreRequestStudentTO" items="${notDnpStudentList}">
						<tr id="row_${rescoreRequestStudentTO.studentBioId}">
							<td class="vertical-center">
								<span class="button blue-gradient glossy float-left margin-left remove-student"
									studentBioId="${rescoreRequestStudentTO.studentBioId}" 
									studentFullName="${rescoreRequestStudentTO.studentFullName}" 
									href="#nogo">Remove</span>
							</td>
						    <td class="vertical-center">${rescoreRequestStudentTO.studentFullName}</td>
						    <td class="vertical-center">
						    	<c:choose>
