...
public interface IReportQuery {
	...
	public static final String INSERT_JOB_TRACKING = CustomStringUtil.appendString(
					...
					"(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, (SELECT ADMINID FROM CUST_PRODUCT_LINK WHERE CUSTOMERID=? AND PRODUCTID=? AND ROWNUM=1), ?, SYSDATE)");
	
	public static final String UPDATE_JOB_TRACKING = "UPDATE JOB_TRACKING SET EXTRACT_ENDDATE=SYSDATE+2, REQUEST_FILENAME=?, REQUEST_SUMMARY=?, JOB_LOG=?, JOB_STATUS=?, UPDATED_DATE_TIME=SYSDATE, FILE_SIZE=? WHERE JOB_ID=?";
