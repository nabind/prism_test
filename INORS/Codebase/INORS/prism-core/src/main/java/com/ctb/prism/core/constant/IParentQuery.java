...
public interface IParentQuery {
	...
	public static final String SEARCH_STUDENT_ON_REDIRECT = CustomStringUtil.appendString(
			...
			" TO_CHAR(STD.STUDENT_BIO_ID) AS ROWIDENTIFIER, STD.TEST_ELEMENT_ID AS TEST_ELEMENT_ID,",
			" GRD.GRADE_NAME AS STUDENTGRADE, IC.ICID AS INVITATIONCODE, IC.ACTIVATION_STATUS AS ACTIVATIONSTATUS, HIER.ORG_NODE_NAME, HIER.ORG_NODEID",
			" FROM ORG_USERS ORG_USR, STUDENT_BIO_DIM STD,  GRADE_DIM GRD, INVITATION_CODE IC, INVITATION_CODE_CLAIM ICC,  ",
			" (SELECT * FROM ORG_NODE_DIM WHERE ORG_MODE = ?  START WITH ORG_NODEID = ",
			" (SELECT PARENT_ORG_NODEID FROM ORG_NODE_DIM WHERE ORG_NODEID = (SELECT ORG_NODEID FROM STUDENT_BIO_DIM WHERE TEST_ELEMENT_ID = ?))",
			" CONNECT BY PRIOR ORG_NODEID = PARENT_ORG_NODEID) HIER ",
			" WHERE HIER.ORG_NODEID = ORG_USR.ORG_NODEID AND ORG_USR.ORG_USER_ID = ICC.ORG_USER_ID AND IC.ACTIVATION_STATUS = 'AC' ",
			" AND ICC.ICID = IC.ICID AND IC.TEST_ELEMENT_ID = STD.TEST_ELEMENT_ID",
			" AND ic.cust_prod_id = ?",
			" AND STD.GRADEID = GRD.GRADEID) ABC",
