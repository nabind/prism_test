...
public interface IParentQuery {
	...
	public static final String INSERT_ORG_USER_PARENT = CustomStringUtil.appendString(
			...
			" (?, ?,(SELECT M.ORG_NODEID FROM INVITATION_CODE M  WHERE M.INVITATION_CODE = ?),'6', ",
	...
	public static final String GET_PARENT_DETAILS_ON_FIRST_LOAD = CustomStringUtil.appendString(
			...
			" WHERE USR.USERID=OU.USERID and OU.ORG_NODEID=HIER.ORG_NODEID AND OU.ORG_NODE_LEVEL = 6 " , " ORDER BY UPPER(USR.USERNAME)) ABC WHERE ROWNUM <= 15 ORDER BY UPPER(ABC.USERNAME)");
	...
	public static final String GET_PARENT_DETAILS_ON_SCROLL = CustomStringUtil.appendString(
			...
			" USR.ACTIVATION_STATUS AS STATUS, USR.LAST_LOGIN_ATTEMPT,HIER.ORG_NODE_NAME, HIER.ORG_NODEID FROM USERS USR,ORG_USERS OU,",
			" (SELECT * FROM ORG_NODE_DIM WHERE ORG_MODE = ?  START WITH ORG_NODEID = ? CONNECT BY PRIOR ORG_NODEID = PARENT_ORG_NODEID) HIER ",
			" WHERE USR.USERID=OU.USERID and OU.ORG_NODEID=HIER.ORG_NODEID AND OU.ORG_NODE_LEVEL = 6 ",
	...
	public static final String GET_PARENT_DETAILS_ON_SCROLL_WITH_SRCH_PARAM = CustomStringUtil.appendString(
					...
					" WHERE USR.USERID=OU.USERID and OU.ORG_NODEID=HIER.ORG_NODEID AND OU.ORG_NODE_LEVEL = 6 AND UPPER(USR.USERNAME) > UPPER(?) AND (UPPER(USR.USERNAME) LIKE UPPER(?) OR UPPER(USR.LAST_NAME) LIKE UPPER(?) OR UPPER(USR.FIRST_NAME) LIKE UPPER(?)) ORDER BY UPPER(USR.USERNAME)) ABC WHERE ROWNUM <= 15 ORDER BY UPPER(ABC.USERNAME)");
	...
	public static final String SEARCH_PARENT = CustomStringUtil.appendString(
			...
			" and OU.ORG_NODEID=HIER.ORG_NODEID AND OU.ORG_NODE_LEVEL = 6 AND (UPPER(USR.USERNAME) LIKE UPPER(?) ",
	...
	/*public static final String SEARCH_STUDENT_ON_REDIRECT = CustomStringUtil.appendString(
			...
			" CONNECT BY PRIOR ORG_NODEID = PARENT_ORG_NODEID) HIER, CUST_PRODUCT_LINK LINK ",
			"  WHERE HIER.ORG_NODEID = ORG_USR.ORG_NODEID AND ORG_USR.ORG_NODE_LEVEL = 6 ",
