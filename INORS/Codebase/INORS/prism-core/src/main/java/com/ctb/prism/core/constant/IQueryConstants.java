 ...
 * @version 1.0
 */
public interface IQueryConstants extends IUserQuery, IOrgQuery, IParentQuery, IReportQuery, IInorsQuery, IEducationQuery, IWSQuery {
	/** test */
	...
	public static final String GET_USER_COUNT = CustomStringUtil.appendString(
			...
			" (SELECT *  FROM ORG_NODE_DIM WHERE CUSTOMERID = ? AND ORG_MODE = ?",
 ...
 // query to insert report role relation in report_role table
	...
	public static final String GET_USER_TYPE = "SELECT DECODE(COUNT(*), 0, 'O', 'P') AS USER_TYPE FROM USERS U, ORG_USERS OU WHERE U.USERID=OU.USERID AND UPPER(U.USERNAME) = UPPER(?) AND OU.ORG_NODE_LEVEL=0";
	...
	public static final String GET_STUDENT_TABLE_GD = CustomStringUtil.appendString(
			...
			" (SELECT STUDENT_BIO_ID, FILENAME FROM STUDENT_PDF_FILES",
	...
	public static final String GET_ALL_STUDENT_TABLE_GD_ALL_GRADES = CustomStringUtil.appendString(
			...
			" 	WHERE PDF_REPORTID IN (SELECT DISTINCT PDF_REPORTID FROM PDF_REPORTS WHERE REPORT_NAME = 'IP')) SPF_IP,",
			" 	(SELECT GRADEID, GRADE_NAME FROM GRADE_DIM) GRADE,",
			" 	(SELECT ORG_NODEID, ORG_NODE_NAME, ORG_NODE_LEVEL FROM ORG_NODE_DIM WHERE ORG_NODE_LEVEL=4 AND ORG_MODE = ? /*'NON-PUBLIC' orgMode*/) CLASS",
			" WHERE SBD.ORG_NODEID IN (SELECT DISTINCT OND.ORG_NODEID",
			...
			" 	WHERE PDT.PRODUCTID  = ? /*3001 productId*/",
			" 	AND OND.ORG_MODE     = ? /*'NON-PUBLIC' orgMode*/",
			...
			" 		AND GSL.ORG_NODEID    = ? /*605247 orgNodeId*/",
			" 		AND CUST.CUSTOMERID   = ? /*1000 customerId*/",
			...
			" 		AND CUST.PRODUCTID    = ? /*3001 productId*/",
			" 		AND GRD.GRADEID       = GSL.GRADEID) A)",
			" 	AND ((? /*orgNodeLevel*/<>4 AND OND.PARENT_ORG_NODEID = ? /*605247 orgNodeId*/) OR (?/*orgNodeLevel*/=4 AND OND.ORG_NODEID = ? /*605247 orgNodeId*/))",
	...
	public static final String GET_IC_FILE_PATH = "SELECT IC_FILE_LOC RESULT FROM INVITATION_CODE WHERE STUDENT_BIO_ID = ? AND IC_FILE_LOC IS NOT NULL";
	...
	public static final String GET_STUDENTS_PDF_FILE_PATH_BOTH = CustomStringUtil.appendString(
			...
			" (SELECT SPF.STUDENT_BIO_ID, SPF.FILENAME, DECODE(SPF.PDF_REPORTID, 1 , 'a-', 2, 'b-', '') PRE,",
			" DECODE(SPF.PDF_REPORTID, 1 , '.ISR', 2, '.IPR', '') EXT",
	...
	public static final String GET_STUDENTS_PDF_FILE_PATH_ISR_ONLY = CustomStringUtil.appendString(
			...
			" DISTRICT.ORG_NODE_NAME || '.' || DISTRICT.ORG_NODEID || '.' ||",
			" SCHOOL.ORG_NODE_NAME || '.' || SCHOOL.ORG_NODEID || '.' ||",
			" GRADE.GRADE_CODE || '.' || FACT.ELEMENT_ FILE_NAME",
			" FROM RESULTS_GRT_FACT FACT, GRADE_DIM GRADE,",
			" (SELECT '?' STUDENT_BIO_ID, ORG_NODEID, ORG_NODE_NAME",
			" FROM ORG_NODE_DIM WHERE ORG_NODE_LEVEL=2",
			" CONNECT BY NOCYCLE ORG_NODEID = PRIOR PARENT_ORG_NODEID",
	...
	public static final String GET_GD_FILE_NAME_DATA = CustomStringUtil.appendString(
			"SELECT FACT.STUDENT_BIO_ID, FACT.ORG_NODEID, FACT.TEST_DATE, FACT.ORGTSTGPGM, GRADE.GRADE_CODE, FACT.ELEMENT_",
	...
	public static final String GET_GD_FILE_NAME = CustomStringUtil.appendString(
			...
			" DISTRICT.ORG_NODE_NAME || '.' || DISTRICT.ORG_NODEID || '.' ||",
			" SCHOOL.ORG_NODE_NAME || '.' || SCHOOL.ORG_NODEID || '.' ||",
			" GRADE.GRADE_CODE || '.' || FACT.ELEMENT_",
			" FROM RESULTS_GRT_FACT FACT, GRADE_DIM GRADE,",
			" (SELECT '?' STUDENT_BIO_ID, ORG_NODEID, ORG_NODE_NAME",
			" FROM ORG_NODE_DIM WHERE ORG_NODE_LEVEL=2",
			" CONNECT BY NOCYCLE ORG_NODEID = PRIOR PARENT_ORG_NODEID",
