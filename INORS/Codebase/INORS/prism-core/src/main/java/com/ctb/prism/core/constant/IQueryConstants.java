...
public interface IQueryConstants extends IUserQuery, IOrgQuery, IParentQuery, IReportQuery, IInorsQuery, IEducationQuery, IWSQuery {
 ...
 // query to insert report role relation in report_role table
	...
	public static final String GET_USER_TYPE = CustomStringUtil.appendString(
			"SELECT DECODE(COUNT(*), 0, 'E', 'O') AS USER_TYPE FROM ORG_USERS WHERE USERID IN (SELECT USERID FROM USERS WHERE UPPER(USERNAME) = UPPER(?)) ",
			"UNION ",
	...
	public static final String GET_TEST_ADMINISTRATIONS_GD = "SELECT * FROM ADMIN_DIM ORDER BY SUBSTR(ADMIN_NAME, -4) DESC, ADMIN_NAME DESC";
	public static final String GET_DISTRICTS_GD = "SELECT DISTINCT ORG_NODEID, ORG_NODE_NAME FROM ORG_NODE_DIM WHERE ORG_NODE_LEVEL = 2 AND ORG_MODE = ? ORDER BY ORG_NODE_NAME";
	public static final String GET_SCHOOLS_GD = "SELECT DISTINCT ORG_NODEID, ORG_NODE_NAME FROM ORG_NODE_DIM WHERE ORG_NODE_LEVEL = 3 AND ORG_MODE = ? AND PARENT_ORG_NODEID = ? ORDER BY ORG_NODE_NAME";
	public static final String GET_CLASSES_GD = "SELECT DISTINCT ORG_NODEID, ORG_NODE_NAME FROM ORG_NODE_DIM WHERE ORG_NODE_LEVEL = 4 AND ORG_MODE = ? AND PARENT_ORG_NODEID = ? ORDER BY ORG_NODE_NAME";
	public static final String GET_GRADES_GD = "SELECT GRADEID, GRADE_NAME FROM GRADE_DIM ORDER BY GRADE_NAME";
	public static final String GET_STUDENT_TABLE_GD = "SELECT STUDENT_BIO_ID, REPLACE(FIRST_NAME || ' ' || MIDDLE_NAME || ' ' || LAST_NAME, '  ', ' ') AS FULL_NAME FROM STUDENT_BIO_DIM WHERE ORG_NODEID = ?";
