...
public interface IQueryConstants extends IUserQuery, IOrgQuery, IParentQuery, IReportQuery, IInorsQuery, IEducationQuery, IWSQuery {
 ...
 // query to insert report role relation in report_role table
	...
	public static final String GET_STUDENT_TABLE_GD = CustomStringUtil.appendString(
			"SELECT SBD.STUDENT_BIO_ID ID, ( SELECT  ORG_NODE_NAME FROM org_node_dim a WHERE a.org_nodeid =sbd.org_nodeid )  CLASS,",
			" SBD.student_LAST_NAME, SBD.student_FIRST_NAME, SBD.STUDENT_MIDDLE_INITIAL MIDDLE_NAME, ",
			...
			" (SELECT STUDENT_BIO_ID, FILENAME, IS_FILE_EXISTS ",
			" FROM STUDENT_PDF_FILES ",
			" WHERE PDF_REPORTID IN (SELECT DISTINCT PDF_REPORTID ",
			" FROM PDF_REPORTS WHERE REPORT_NAME = 'ISR' AND cust_prod_id = ?)) SPF_ISR, ",
			" (SELECT STUDENT_BIO_ID, FILENAME, IS_FILE_EXISTS ",
			"  FROM STUDENT_PDF_FILES ",
			" WHERE PDF_REPORTID IN (SELECT DISTINCT PDF_REPORTID ",
