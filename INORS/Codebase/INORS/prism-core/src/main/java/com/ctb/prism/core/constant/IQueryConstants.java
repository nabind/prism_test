...
public interface IQueryConstants extends IUserQuery, IOrgQuery, IParentQuery, IReportQuery, IInorsQuery, IEducationQuery, IWSQuery {
	...
	public static final String GET_SYSTEM_CONFIGURATION_MESSAGE_MORE_INFO =  CustomStringUtil.appendString(
			...
			" JOIN DASH_MESSAGES DM on DMRA.DB_REPORTID=DM.DB_REPORTID ",
			" JOIN DASH_MESSAGE_TYPE DMT on  DM.MSG_TYPEID=DMT.MSG_TYPEID ",
			" where DR.DB_REPORTID= ? and DMT.MESSAGE_NAME= ? and DM.ACTIVATION_STATUS='AC' and ROWNUM=1");
 ...
 // query to insert report role relation in report_role table
	...
	public static final String GET_STUDENT_TABLE_GD = "SELECT STUDENT_BIO_ID VALUE, REPLACE(FIRST_NAME || ' ' || MIDDLE_NAME || ' ' || LAST_NAME, '  ', ' ') AS NAME, ORG_NODEID OTHER FROM STUDENT_BIO_DIM WHERE ORG_NODEID = ?";
	public static final String GET_ALL_STUDENT_TABLE_GD = "SELECT STUDENT_BIO_ID VALUE, REPLACE(FIRST_NAME || ' ' || MIDDLE_NAME || ' ' || LAST_NAME, '  ', ' ') AS NAME, ORG_NODEID OTHER FROM STUDENT_BIO_DIM WHERE ORG_NODEID IN (SELECT DISTINCT ORG_NODEID FROM ORG_NODE_DIM WHERE PARENT_ORG_NODEID = ?)";
