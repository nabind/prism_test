...
public interface IQueryConstants extends IUserQuery, IOrgQuery, IParentQuery, IReportQuery, IInorsQuery, IEducationQuery, IWSQuery, IRescoreRequestQuery {
 ...
 // query to insert report role relation in report_role table
	...
	/*public static final String GET_STUDENT_TABLE_GD = CustomStringUtil.appendString(
			...
			" IC.ACTIVATION_STATUS IC_FLAG, DECODE(IC.FILENAME, NULL, ' ', IC.FILENAME) IC,",
			" SPF_ISR.IS_FILE_EXISTS ISR_FLAG, DECODE(SPF_ISR.FILENAME, NULL, ' ', SPF_ISR.FILENAME) ISR,",
			" SPF_IP.IS_FILE_EXISTS IP_FLAG, DECODE(SPF_IP.FILENAME, NULL, ' ', SPF_IP.FILENAME) IP",
			" FROM STUDENT_BIO_DIM SBD, INVITATION_CODE IC,",
			" (SELECT STUDENT_BIO_ID, FILENAME, IS_FILE_EXISTS FROM STUDENT_PDF_FILES",
			" WHERE PDF_REPORTID IN (SELECT DISTINCT PDF_REPORTID FROM PDF_REPORTS WHERE REPORT_NAME = 'ISR')) SPF_ISR,",
			" (SELECT STUDENT_BIO_ID, FILENAME, IS_FILE_EXISTS FROM STUDENT_PDF_FILES",
			" WHERE PDF_REPORTID IN (SELECT DISTINCT PDF_REPORTID FROM PDF_REPORTS WHERE REPORT_NAME = 'IP')) SPF_IP,",
			" (SELECT GRADEID,GRADE_NAME FROM GRADE_DIM) GRADE,",
			" (SELECT ORG_NODEID, ORG_NODE_NAME, ORG_NODE_LEVEL FROM ORG_NODE_DIM WHERE ORG_NODE_LEVEL=4 and org_mode = ?) CLASS",
			" WHERE SBD.ORG_NODEID = ?",
			...
			" AND GRADE.GRADEID = ?",
			" AND SBD.GRADEID=GRADE.GRADEID",
			" AND SBD.ORG_NODEID=CLASS.ORG_NODEID",
			" #"
			);*/
	...
	/*public static final String GET_STUDENT_TABLE_GD = CustomStringUtil.appendString(
			...
			" IC.ACTIVATION_STATUS IC_FLAG, DECODE(IC.FILENAME, NULL, ' ', IC.FILENAME) IC,",
			" (SELECT S.IS_FILE_EXISTS FROM STUDENT_PDF_FILES S, PDF_REPORTS P",
			" WHERE REPORT_NAME = 'ISR' AND CUST_PROD_ID = ?",
			" AND P.PDF_REPORTID = S.PDF_REPORTID AND S.STUDENT_BIO_ID = SBD.STUDENT_BIO_ID) AS ISR_FLAG,",
			" DECODE((SELECT S.FILENAME FROM STUDENT_PDF_FILES S, PDF_REPORTS P",
			" WHERE REPORT_NAME = 'ISR' AND CUST_PROD_ID = ?",
			" AND P.PDF_REPORTID = S.PDF_REPORTID AND S.STUDENT_BIO_ID = SBD.STUDENT_BIO_ID), NULL, ' ',",
			" (SELECT S.FILENAME FROM STUDENT_PDF_FILES S, PDF_REPORTS P",
			" WHERE REPORT_NAME = 'ISR' AND CUST_PROD_ID = ?",
			" AND P.PDF_REPORTID = S.PDF_REPORTID AND S.STUDENT_BIO_ID = SBD.STUDENT_BIO_ID)) ISR,",
			" (SELECT S.IS_FILE_EXISTS FROM STUDENT_PDF_FILES S, PDF_REPORTS P",
			" WHERE P.REPORT_NAME = 'IP' AND P.CUST_PROD_ID = ?",
			" AND P.PDF_REPORTID = S.PDF_REPORTID AND S.STUDENT_BIO_ID = SBD.STUDENT_BIO_ID) AS IP_FLAG,",
			" DECODE((SELECT S.FILENAME FROM STUDENT_PDF_FILES S, PDF_REPORTS P",
			" WHERE P.REPORT_NAME = 'IP' AND P.CUST_PROD_ID = ?",
			" AND P.PDF_REPORTID = S.PDF_REPORTID AND S.STUDENT_BIO_ID = SBD.STUDENT_BIO_ID), NULL, ' ',",
			" (SELECT S.FILENAME FROM STUDENT_PDF_FILES S, PDF_REPORTS P",
			" WHERE P.REPORT_NAME = 'IP' AND P.CUST_PROD_ID = ?",
			" AND P.PDF_REPORTID = S.PDF_REPORTID AND S.STUDENT_BIO_ID = SBD.STUDENT_BIO_ID)) IP",
			" FROM RESULTS_GRT_FACT SBD, INVITATION_CODE IC",
			" WHERE SBD.ORG_NODEID = ?", // Main WHERE, Previous lines of the query are same for all 4 queries
			" AND SBD.GRADEID = ? ",
...
/*	public static final String GET_STUDENT_TABLE_GD_ALL_GRADES = CustomStringUtil.appendString(
	...
	/*public static final String GET_ALL_STUDENT_TABLE_GD = CustomStringUtil.appendString(
			...
			" AND P.PDF_REPORTID = S.PDF_REPORTID AND S.STUDENT_BIO_ID = SBD.STUDENT_BIO_ID)) IP",
			" FROM RESULTS_GRT_FACT SBD, INVITATION_CODE IC",
			" WHERE SBD.ORG_NODEID IN (SELECT VC1 AS ORG_NODEID FROM TABLE(SELECT SF_GET_CLASS(?, ?, ?, ?, ?, ?, ?, 'ADD_ALL') FROM DUAL))", // Main WHERE, Previous lines of the query are same for all 4 queries
			" AND SBD.GRADEID = ? ",
			" AND SBD.CUST_PROD_ID = ? ",
			" AND ISPUBLIC = ? ",
			" AND SBD.STUDENT_BIO_ID = IC.STUDENT_BIO_ID(+)",
