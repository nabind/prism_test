...
public interface IQueryConstants extends IUserQuery, IOrgQuery, IParentQuery, IReportQuery, IInorsQuery, IEducationQuery, IWSQuery {
	...
	public static final String GET_SYSTEM_CONFIGURATION_MESSAGE_MORE_INFO =  CustomStringUtil.appendString(
			"select DM.REPORT_MSG as REPORT_MSG from DASH_REPORTS DR ",
			" join DASH_MENU_RPT_ACCESS DMRA on DR.DB_REPORTID=DMRA.DB_REPORTID ",
			" JOIN DASH_MESSAGES DM on DMRA.DB_REPORTID=DM.DB_REPORTID ",
			" JOIN DASH_MESSAGE_TYPE DMT on  DM.MSG_TYPEID=DMT.MSG_TYPEID ",
 ...
 // query to insert report role relation in report_role table
	...
	public static final String GET_STUDENT_TABLE_GD = CustomStringUtil.appendString(
			...
			" CLASS.ORG_NODE_NAME CLASS,GRADE.GRADE_NAME GRADE,",
			" DECODE(IC.IC_FILE_LOC, NULL, ' ') IC,",
			" DECODE(SPF_ISR.FILENAME, NULL, ' ') ISR,",
			" DECODE(SPF_IP.FILENAME, NULL, ' ') IP",
			...
			" (SELECT ORG_NODEID, ORG_NODE_NAME, ORG_NODE_LEVEL FROM ORG_NODE_DIM WHERE ORG_NODE_LEVEL=4 ) CLASS",
			" WHERE SBD.STUDENT_BIO_ID=IC.STUDENT_BIO_ID(+)",
			...
			" AND SBD.ORG_NODEID = ? ORDER BY NAME"
			);
	public static final String GET_ALL_STUDENT_TABLE_GD = CustomStringUtil.appendString(
			...
			" AND SBD.GRADEID=GRADE.GRADEID",
			" AND SBD.ORG_NODEID=CLASS.ORG_NODEID",
			" AND SBD.ORG_NODEID IN (SELECT DISTINCT ORG_NODEID FROM ORG_NODE_DIM WHERE PARENT_ORG_NODEID = ?) ORDER BY NAME"
			);
