...
public interface IQueryConstants extends IUserQuery, IOrgQuery, IParentQuery, IReportQuery, IInorsQuery, IEducationQuery, IWSQuery {
 ...
 // query to insert report role relation in report_role table
	...
	public static final String GET_STUDENT_TABLE_GD = CustomStringUtil.appendString(
			"SELECT SBD.STUDENT_BIO_ID ID, REPLACE(SBD.FIRST_NAME || ' ' || SBD.MIDDLE_NAME || ' ' || SBD.LAST_NAME, '  ', ' ') AS NAME,",
			" DECODE(IC.IC_FILE_LOC, NULL, ' ') IC, DECODE(SPF_ISR.FILENAME, NULL, ' ') ISR, DECODE(SPF_IP.FILENAME, NULL, ' ') IP",
			" FROM STUDENT_BIO_DIM SBD, INVITATION_CODE IC, (SELECT STUDENT_BIO_ID, FILENAME FROM STUDENT_PDF_FILES",
	...
	public static final String GET_ALL_STUDENT_TABLE_GD = CustomStringUtil.appendString(
			...
			" AND SBD.STUDENT_BIO_ID=SPF_ISR.STUDENT_BIO_ID(+)",
			" AND SBD.STUDENT_BIO_ID=SPF_IP.STUDENT_BIO_ID(+)",
			" AND SBD.ORG_NODEID IN (SELECT DISTINCT ORG_NODEID FROM ORG_NODE_DIM WHERE PARENT_ORG_NODEID = ?)"
			);
