CREATE OR REPLACE FUNCTION SF_GET_GENERATE_FILE (
...
CURSOR c_Get_File_Type (ip_product_type VARCHAR)
  IS
  SELECT
  COLUMN_VALUE FILE_CODE
  ,DECODE(COLUMN_VALUE,'ISR','Individual Student Report (ISR)','IPR','Image Print','BOTH','Both (ISR and Image Print)','Invitation Letter') AS FILE_NAME
  FROM TABLE(dyn_attr_dtls('ISR','IPR','BOTH','ICL'))
  WHERE 'ISTEP' =ip_product_type
  UNION ALL
  SELECT
  COLUMN_VALUE FILE_CODE
  ,DECODE(COLUMN_VALUE,'ISR','Individual Student Report (ISR)'/*,'Invitation Letter'*/) AS FILE_NAME
...
CURSOR c_Get_ProductCode_Default
  IS
...
CURSOR c_Get_ProductCode_Cascade (ip_test_administration PRODUCT.PRODUCTID%TYPE )
  IS
 (SELECT SUBSTR(PDT.PRODUCT_CODE, 1,5) AS PRODUCT_CODE
                                        FROM PRODUCT PDT
...
BEGIN
     ...
     IF  p_test_administration =-99 OR p_test_administration IS NULL  THEN
         FOR r_Get_ProductCode_Default IN c_Get_ProductCode_Default
         LOOP
          v_ProductCode := r_Get_ProductCode_Default.PRODUCT_CODE;
         END LOOP;
     ELSE
         FOR r_Get_ProductCode_Cascade IN c_Get_ProductCode_Cascade(p_test_administration)
         LOOP
          v_ProductCode := r_Get_ProductCode_Cascade.PRODUCT_CODE;
         END LOOP;
     END IF;

     FOR r_Get_File_Type IN c_Get_File_Type (v_ProductCode)
      LOOP
       t_PRS_PGT_GLOBAL_TEMP_OBJ := PRS_PGT_GLOBAL_TEMP_OBJ();
