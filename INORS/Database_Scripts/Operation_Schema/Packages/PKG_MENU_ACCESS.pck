...
BEGIN
  ...
  WHERE DMRA.DB_MENUID = DM.DB_MENUID
    ...
    AND EXISTS (SELECT 1 FROM USER_ROLE a WHERE a.ROLEID = DMRA.ROLEID 
     ...
     AND (EXISTS (SELECT 1 FROM ORG_USERS a WHERE a.ORG_NODE_LEVEL = DMRA.ORG_LEVEL AND a.USERID = P_IN_USERID )
			OR (EXISTS (SELECT 1 FROM EDU_CENTER_USER_LINK a WHERE a.USERID = P_IN_USERID)))
    AND DR.ACTIVATION_STATUS = 'AC'
