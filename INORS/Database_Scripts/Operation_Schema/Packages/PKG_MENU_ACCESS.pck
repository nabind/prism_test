...
BEGIN
  ...
  AND ACC.ROLEID IN (SELECT ROLEID FROM USER_ROLE WHERE USERID = P_IN_USERID)
...
BEGIN
  ...
  AND ACC.DB_REPORTID       = RE.DB_REPORTID
  AND RE.ACTIVATION_STATUS IN ('AC','SS')
  AND NOT EXISTS (SELECT 1 FROM DASH_MENU_RPT_ACCESS R WHERE R.DB_REPORTID = ACC.DB_REPORTID AND R.ROLEID = P_IN_ROLEID)
  AND ACC.ORG_LEVEL = P_IN_ORG_NODE_LEVEL
  ORDER BY ASS.MENU_SEQ, ACC.REPORT_SEQ, RE.REPORT_NAME DESC;
EXCEPTION
WHEN OTHERS THEN
