...
BEGIN
	OPEN P_OUT_REPORT_CURSOR FOR
		...
		(SELECT DISTINCT ORG_LEVEL,ORG_LABEL FROM ORG_TP_STRUCTURE UNION SELECT -99, 'EDUCATION CENTER' FROM DUAL) OTS
	...
	OPEN P_OUT_ACTION_CURSOR FOR
		...
		(SELECT DISTINCT ORG_LEVEL,ORG_LABEL FROM ORG_TP_STRUCTURE) OTS
		WHERE DAA.DB_REPORTID=502
