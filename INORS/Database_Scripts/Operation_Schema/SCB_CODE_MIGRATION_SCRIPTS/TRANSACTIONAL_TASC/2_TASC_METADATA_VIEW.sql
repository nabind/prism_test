--CREATE VIEW FOR METADATA IN TASC
...
---Created MATERIALIZED VIEW for performance improvement
CREATE MATERIALIZED VIEW CUSTOMER_INFO
...
-- DROP VIEW FORM_DIM
--created MATERIALIZED VIEW  for performance improvement
  CREATE MATERIALIZED VIEW FORM_DIM 
  REFRESH COMPLETE ON DEMAND
  AS
 SELECT FORMID, FORM_NAME, FORM_CODE, DATETIMESTAMP
   FROM PRISMGLOBAL.FORM_DIM
...
SELECT GENDERID, GENDER_NAME, GENDER_CODE, DATETIMESTAMP
  FROM prismglobal.GENDER_DIM
 WHERE PROJECTID = 1;*/
 --DROP VIEW GENDER_DIM
 ---created MATERIALIZED VIEW for performanxce improvement
CREATE MATERIALIZED VIEW GENDER_DIM 
  REFRESH COMPLETE ON DEMAND
  AS
 SELECT GENDERID, GENDER_NAME, GENDER_CODE, DATETIMESTAMP
   FROM PRISMGLOBAL.GENDER_DIM
...
/* CREATE OR REPLACE VIEW OBJECTIVE_DIM AS
 SELECT OBJECTIVEID,
        ...
        OBJECTIVE_DESC,
        DATETIMESTAMP
   FROM PRISMGLOBAL.OBJECTIVE_DIM
  WHERE PROJECTID = 1;*/
   -- DROP VIEW  OBJECTIVE_DIM
 ---created MATERIALIZED VIEW for performance improvement
CREATE MATERIALIZED VIEW  OBJECTIVE_DIM 
...
SELECT ASSESSMENTID,
       ...
       PRODUCTID,
       DATETIMESTAMP
  FROM prismglobal.ASSESSMENT_DIM
...
SELECT CONTENTID, CONTENT_NAME, CONTENT_SEQ, ASSESSMENTID, DATETIMESTAMP
 ...
 CREATE OR REPLACE VIEW GRADE_LEVEL_MAP AS
    ...
    ONLEVEL_FLAG,
    DATETIMESTAMP
      FROM prismglobal.GRADE_LEVEL_MAP
     WHERE LEVEL_MAPID IN
           (SELECT LEVEL_MAPID
              FROM prismglobal.LEVEL_MAP
...
--DROP MATERIALIZED VIEW  SUBTEST_DIM
---created MATERIALIZED VIEW for performance improvemment
CREATE MATERIALIZED VIEW  SUBTEST_DIM 
REFRESH COMPLETE ON DEMAND
AS
SELECT SUBTESTID,
       SUBTEST_NAME,
...
SELECT SUBT_OBJ_MAPID,
       ...
       ASSESSMENTID,
       DATETIMESTAMP
  FROM prismglobal.SUBTEST_OBJECTIVE_MAP
 WHERE ASSESSMENTID IN (SELECT ASSESSMENTID FROM ASSESSMENT_DIM);
...
SELECT DB_ACT_ACCESSID,
       ...
       CREATED_DATE_TIME,
       UPDATED_DATE_TIME
  FROM prismglobal.DASH_ACTION_ACCESS
 WHERE CUST_PROD_ID IN
       (SELECT CUST_PROD_ID
          FROM prismglobal.CUST_PRODUCT_LINK
         WHERE PRODUCTID IN
               (SELECT PRODUCTID FROM prismglobal.PRODUCT WHERE PROJECTID = 1));
...
---DROP VIEW CUST_PRODUCT_LINK;	   
---created MATERIALIZED VIEW as a part of performance improvement	
   
CREATE MATERIALIZED VIEW CUST_PRODUCT_LINK 
REFRESH COMPLETE ON DEMAND
AS
SELECT CUST_PROD_ID,
       CUSTOMERID,
...
SELECT TP_ID,
 ...
 WHERE CUSTOMERID IN
       (SELECT CUSTOMERID FROM prismglobal.CUSTOMER_INFO WHERE PROJECTID = 1)
   AND ADMINID IN
       (SELECT ADMINID FROM prismglobal.ADMIN_DIM WHERE PROJECTID = 1);
...
SELECT ROLEID, CUSTOMERID, CREATED_DATE_TIME, UPDATED_DATE_TIME
  FROM prismglobal.ROLE_CUSTOMER
...
SELECT DB_MENUID,
       ...
       CREATED_DATE_TIME,
       UPDATED_DATE_TIME
  FROM prismglobal.DASH_MENU_RPT_ACCESS
...
SELECT MSG_TYPEID,
       ...
       CREATED_DATE_TIME,
       UPDATED_DATE_TIME
  FROM prismglobal.DASH_MESSAGE_TYPE
 WHERE CUST_PROD_ID IN (SELECT CUST_PROD_ID FROM CUST_PRODUCT_LINK);
...
SELECT DB_REPORTID,
       ...
       CREATED_DATE_TIME,
       UPDATED_DATE_TIME
  FROM prismglobal.DASH_MESSAGES
 WHERE CUST_PROD_ID IN (SELECT CUST_PROD_ID FROM CUST_PRODUCT_LINK);
...
SELECT CONDCODE_ID,
       ...
       CREATED_DATE_TIME,
       UPDATED_DATE_TIME
  FROM prismglobal.CONDITION_CODES
...
SELECT ROLEID,
       ...
       CUSTOMERID,
       DATETIMESTAMP
  FROM prismglobal.ORG_USER_DEFINE_LOOKUP
...
SELECT MAPPING_NAME,
       ...
       CREATED_DATE_TIME,
       UPDATED_DATE_TIME
  FROM prismglobal.ETL_JOBMASTER_CONFIG
 WHERE PROJECTID = 1;
...
SELECT SEQ_NO,
       ...
       PRODUCT_CODE,
       DATETIMESTAMP
  FROM prismglobal.ETL_PROJECT_CONFIG
 WHERE PROJECTID = 1;
