...
CREATE OR REPLACE PACKAGE BODY PKG_ISTEP_MATRIX AS
  ...
  BEGIN
    SELECT CUST_PROD_ID
      INTO V_CURR_CUST_PROD_ID
      FROM (SELECT SCR.CUST_PROD_ID
              FROM GRW_SUBTEST_SCORE_FACT SCR
             WHERE SCR.CUST_PROD_ID = P_CUST_PROD_ID
               AND ROWNUM = 1
            UNION ALL
            SELECT -1 AS CUST_PROD_ID FROM DUAL ORDER BY 1 DESC)
     WHERE ROWNUM = 1;
    ...
    IF V_CURR_CUST_PROD_ID = P_CUST_PROD_ID THEN
      INSERT INTO CHECK_RECS
        (TYPE1, TIMETO)
      VALUES
        ('Start Delete GRW_SUBTEST_SCORE_FACT for' || P_CUST_PROD_ID,
         SYSDATE);
      DELETE FROM GRW_SUBTEST_SCORE_FACT
       WHERE CUST_PROD_ID = P_CUST_PROD_ID;
      INSERT INTO CHECK_RECS
        (TYPE1, TIMETO)
      VALUES
        ('End Delete GRW_SUBTEST_SCORE_FACT for' || P_CUST_PROD_ID,
         SYSDATE);

      INSERT INTO CHECK_RECS
        (TYPE1, TIMETO)
      VALUES
        ('Start Delete GRW_DUPLICATE_STUDENTTS for' || P_CUST_PROD_ID,
         SYSDATE);
      DELETE FROM GRW_DUPLICATE_STUDENTS
       WHERE CUST_PROD_ID = P_CUST_PROD_ID;
      INSERT INTO CHECK_RECS
        (TYPE1, TIMETO)
      VALUES
        ('End Delete GRW_DUPLICATE_STUDENTTS for' || P_CUST_PROD_ID,
         SYSDATE);
      COMMIT;
    END IF;

    FOR R_PREV_CUST_PROD_ID IN C_PREV_CUST_PROD_ID LOOP
    ...
    SELECT CUST_PROD_ID
      ...
      FROM (SELECT SCR.CUST_PROD_ID
              FROM GRW_SUBTEST_SCORE_FACT SCR
             WHERE SCR.CUST_PROD_ID = V_PREV_CUST_PROD_ID
               AND ROWNUM = 1
            UNION ALL
            SELECT -1 AS CUST_PROD_ID FROM DUAL ORDER BY 1 DESC)
     WHERE ROWNUM = 1;
    ...
    IF V_CNT > 0 THEN
      INSERT INTO CHECK_RECS
        (TYPE1, TIMETO)
      VALUES
        ('Start Delete GRW_SUBTEST_SCORE_FACT for' || V_PREV_CUST_PROD_ID,
         SYSDATE);
      DELETE FROM GRW_SUBTEST_SCORE_FACT
       WHERE CUST_PROD_ID = V_PREV_CUST_PROD_ID;
      INSERT INTO CHECK_RECS
        (TYPE1, TIMETO)
      VALUES
        ('End Delete GRW_SUBTEST_SCORE_FACT for' || V_PREV_CUST_PROD_ID,
         SYSDATE);

      INSERT INTO CHECK_RECS
        (TYPE1, TIMETO)
      VALUES
        ('Start Delete GRW_DUPLICATE_STUDENTTS for' || V_PREV_CUST_PROD_ID,
         SYSDATE);
      DELETE FROM GRW_DUPLICATE_STUDENTS
       WHERE CUST_PROD_ID = V_PREV_CUST_PROD_ID;
      INSERT INTO CHECK_RECS
        (TYPE1, TIMETO)
      VALUES
        ('End Delete GRW_DUPLICATE_STUDENTTS for' || V_PREV_CUST_PROD_ID,
         SYSDATE);
      COMMIT;
      --V_CNT :=0;
    END IF;
    ...
    IF IS_SPLIT_PARTITION = 'Y' THEN
      FOR R_GRW_SUBTEST_PARTITION IN C_GRW_SUBTEST_PARTITION LOOP
        IF R_GRW_SUBTEST_PARTITION.CUST_PROD_ID <> TO_CHAR(P_CUST_PROD_ID) THEN
          EXECUTE IMMEDIATE 'ALTER TABLE GRW_SUBTEST_SCORE_FACT
                        ...
                        VALUES (' ||
                            P_CUST_PROD_ID || ')
                        INTO ( PARTITION PART_C' ||
                            P_CUST_PROD_ID || ', PARTITION PART_OTHERS )';

          INSERT INTO CHECK_RECS
            (TYPE1, TIMETO)
          VALUES
            ('Partition created GRW_SUBTEST_SCORE_FACT for' ||
             P_CUST_PROD_ID,
             SYSDATE);
    ...
    INSERT INTO GRW_DUPLICATE_STUDENTS
      SELECT A.*
        FROM (SELECT COUNT(GRT.STUDENT_BIO_ID) OVER(PARTITION BY GRT.CUST_PROD_ID, GRT.STUDENT_TEST_AI) AS CNT,
                     GRT.STUDENT_BIO_ID,
                     GRT.CUST_PROD_ID,
                     GRT.STUDENT_TEST_AI,
                     GRT.ADMINID,
               ...
               WHERE GRT.CUST_PROD_ID IN
                     (P_CUST_PROD_ID, V_PREV_CUST_PROD_ID)) A
       WHERE A.CNT > 1;
    COMMIT;
    ...
    INSERT INTO CHECK_RECS
      (TYPE1, TIMETO)
    VALUES
      ('Data inserted into GRW_SUBTEST_SCORE_FACT for ' || P_CUST_PROD_ID || ', ' ||
       V_PREV_CUST_PROD_ID,
       SYSDATE);
    COMMIT;
  ...
  BEGIN
    ...
    IF P_QUERY_TYP = 'ALL' THEN
      FOR J IN (SELECT 'ELA' SUB
                  FROM DUAL
                UNION ALL
                SELECT 'MATH' SUB FROM DUAL) LOOP
    ...
    ELSE
      ...
      FOR J IN (SELECT 'ELA' SUB
                ...
                UNION ALL
                SELECT 'MATH' SUB FROM DUAL) LOOP
        ISTEP_MATRIX_REPORT_LOAD(P_QUERY_TYP,
                                 P_IS_FIRST,
                                 P_CUST_PROD_ID,
                                 J.SUB);
      END LOOP;
  ...
  PROCEDURE ISTEP_MATRIX_REPORT_LOAD(P_QUERY_TYP    VARCHAR2,
    ...
    CURSOR C_USERID IS
    --SELECT USR.USERID FROM USERS USR WHERE USER_TYPE = 'GRW';
      SELECT USR.USERID
  ...
  BEGIN
    ...
    SELECT PRODUCTID
      INTO V_PRODUCTID_PREV
      FROM PRODUCT
     WHERE PRODUCT_CODE =
           (SELECT SUBSTR(PDT.PRODUCT_CODE, 1, LENGTH(PDT.PRODUCT_CODE) - 2) ||
                   SUBSTR(ADM.ADMIN_YEAR - 1, -2, 2) AS PREV_PRODUCT_CODE
              FROM PRODUCT PDT, ADMIN_DIM ADM, CUST_PRODUCT_LINK CPL
             WHERE CPL.CUST_PROD_ID = P_CUST_PROD_ID
               AND CPL.ADMINID = ADM.ADMINID
    ...
    LOOP
      FETCH C_USERID BULK COLLECT
        INTO LV_TYP_VARCHAR_ARR LIMIT 50;
        ...
        FORALL I IN LV_TYP_VARCHAR_ARR.FIRST .. LV_TYP_VARCHAR_ARR.LAST
          DELETE FROM PERF_MATRIX_FACT
           WHERE USER_ID = LV_TYP_VARCHAR_ARR(I)
             ...
             AND SUBTESTID = V_SUBTESTID;
        COMMIT;
        INSERT INTO CHECK_RECS
          (TYPE1, TIMETO)
        VALUES
          ('End Delete ' || P_QUERY_TYP || ' Subtest: ' || P_SUBTESTCODE,
           SYSDATE);
      END IF;
      ...
      INSERT INTO CHECK_RECS
        (TYPE1, TIMETO)
      VALUES
        ('Start ' || P_QUERY_TYP || '| Subtest: ' || P_SUBTESTCODE,
         SYSDATE);
      ...
      INSERT INTO MATRIX_SELECTION_LOOKUP_1
        ...
        DISTINCT USR.USERID,
          ...
          FROM USERS                 USR,
               ...
               CUST_PRODUCT_LINK     CPL
         WHERE USR.USERID IN
               (SELECT COLUMN_VALUE FROM TABLE(LV_TYP_VARCHAR_ARR))
           ...
           AND USC.CUST_PROD_ID = USL.CUST_PROD_ID
      ...
      INSERT INTO CHECK_RECS
        (TYPE1, TIMETO)
      VALUES
        ('End---MATRIX_SELECTION_LOOKUP_1|' || ' Subtest: ' ||
         P_SUBTESTCODE,
         SYSDATE);
      ...
      INSERT INTO CHECK_RECS
        (TYPE1, TIMETO)
      VALUES
        ('Start---MATRIX_SELECTION_LOOKUP_2|' || ' Subtest: ' ||
         P_SUBTESTCODE,
         SYSDATE);
      ...
      INSERT INTO CHECK_RECS
        (TYPE1, TIMETO)
      VALUES
        ('End---MATRIX_SELECTION_LOOKUP_2|' || ' Subtest: ' ||
         P_SUBTESTCODE,
         SYSDATE);
      INSERT INTO CHECK_RECS
        (TYPE1, TIMETO)
      VALUES
        ('Start---MATRIX_SELECTION_LOOKUP_3|' || ' Subtest: ' ||
         P_SUBTESTCODE,
         SYSDATE);
      ...
      INSERT INTO MATRIX_SELECTION_LOOKUP_3
        ...
        /*(SELECT A.STUDENT_BIO_ID
         ...
         WHERE STD.STUDENT_BIO_ID = M.STUDENT_BIO_ID
           AND M.SUBTESTID = V_SUBTESTID --R_SUBTESTID.SUBTESTID
      ...
      INSERT INTO CHECK_RECS
        (TYPE1, TIMETO)
      VALUES
        ('End---MATRIX_SELECTION_LOOKUP_3|' || ' Subtest: ' ||
         P_SUBTESTCODE,
         SYSDATE);
      ...
      INSERT INTO CHECK_RECS
        (TYPE1, TIMETO)
      VALUES
        ('Start---PERF_MATRIX_FACT|' || ' Subtest: ' || P_SUBTESTCODE,
         SYSDATE);
      ...
      WHERE CURR_SUBTEST.SUBTEST_CODE = PREV_SUBTEST.SUBTEST_CODE
        ...
        AND STD.CUSTOMERID =  V_CUSTOMER_ID
      ...
      INSERT INTO PERF_MATRIX_FACT
        SELECT PERF_MATRIX_SEQ.NEXTVAL,
               (SELECT ORG_NODEID
                  FROM STUDENT_BIO_DIM A
                 WHERE A.STUDENT_BIO_ID = CURR.STUDENT_BIO_ID) AS ORG_NODEID,
               (SELECT CUST_PROD_ID
                  FROM RESULTS_GRT_FACT A
                 WHERE A.STUDENT_BIO_ID = CURR.STUDENT_BIO_ID) AS CUST_PROD_ID,
               CURR.STDUENT_NAME AS STDUENT_NAME,
               CURR.STUDENT_BIO_ID,
               STD.STUDENT_BIO_ID,
               --CURR.PREV_STUDENT_BIO_ID,
               ...
               ADM.ADMINID AS PREV_ADMINID,
               (SELECT SS
                  FROM GRW_SUBTEST_SCORE_FACT SC
                 WHERE SC.STUDENT_BIO_ID = CURR.STUDENT_BIO_ID
                   AND SC.SUBTESTID = CURR_SUBTEST.SUBTESTID
                   AND SC.GRADEID = CURR.GRADEID
                   AND SC.ADMINID = CURR.ADMINID
               ...
               (SELECT USERID
                  FROM USERS USR
                 WHERE USR.USERNAME = CURR.USERNAME) USERID,
               P_QUERY_TYP AS PERF_LEVEL,
               SYSDATE
        --SYSDATE
          FROM MATRIX_SELECTION_LOOKUP_3 CURR,
         ...
         WHERE /*CURR_SUBTEST.SUBTEST_CODE = PREV_SUBTEST.SUBTEST_CODE
                                       AND */
         CURR_SUBTEST.SUBTESTID = CURR.SUBTESTID
        /* AND PREV_SUBTEST.CONTENTID IN
                                           (SELECT CONTENTID
                                              FROM CONTENT_DIM
                                             WHERE ASSESSMENTID IN
        ...
        --AND SCR.SUBTESTID = PREV_SUBTEST.SUBTESTID
         AND SCR.SUBTESTID =
         (SELECT PREV_SUBTEST.SUBTESTID
            FROM SUBTEST_DIM PREV_SUBTEST
           WHERE CURR_SUBTEST.SUBTEST_CODE = PREV_SUBTEST.SUBTEST_CODE
             AND PREV_SUBTEST.CONTENTID IN
                 (SELECT CONTENTID
                    FROM CONTENT_DIM
  ...
  BEGIN
    ...
    IF P_IS_FIRST = 'Y' THEN
      ...
      SELECT COUNT(1)
        INTO L_COUNT
        FROM ALL_INDEXES A
       WHERE A.INDEX_NAME = 'IDX_PERF_MATRIX_FACT_1';
      IF L_COUNT = 1 THEN
      ...
      SELECT COUNT(1)
        INTO L_COUNT
        FROM ALL_INDEXES A
       WHERE A.INDEX_NAME = 'IDX_PERF_MATRIX_FACT_2';
      IF L_COUNT = 1 THEN
