...
var resourceReport = {
    ...
    _initEvents: function() {
        ...
        jQuery("#"+this.FILE_UPLOAD_BUTTON_ID).click(_.bind(function(e){
            e.preventDefault();
            jQuery("#"+this.FILE_PATH_ID).trigger('click');
        }, this));
        ...
        this._filePath.observe('change', function(){
        	if (isIE()){
        		...
        		if (this.value.toLowerCase().indexOf('c:\\fakepath\\') != -1){
        			that._fileName.value = this.value.substring('c:\\fakepath\\'.length,this.value.length);
        		}
        		else{
        			that._fileName.value = this.value;
        		}
        	}
        	else{
        		that._fileName.value = this.files[0].name;
            }
