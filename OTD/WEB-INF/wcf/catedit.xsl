...
<xsl:template match="cat-category">
  <tr>
    <th align="left" class="navi-axis">
      <img src="{$context}/wcf/catedit/{@icon}" onclick="pageAlert=false" width="9" height="9"/>
      <xsl:text> </xsl:text>
...
<xsl:template match="cat-button">
  <img src="{$context}/wcf/catedit/empty.png" onclick="pageAlert=false" width="9" height="9"/>
  <xsl:text> </xsl:text>
</xsl:template>
