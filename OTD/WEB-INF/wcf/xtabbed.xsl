...
<xsl:template match="xpanel[@current='true']" mode="tabbed-heading">
  <td class="tabbed-current">
    <img src="{$context}/wcf/tabbed/current.png" onclick="pageAlert=false"/>
    <xsl:text> </xsl:text>
...
<xsl:template match="xpanel" mode="tabbed-heading">
  <td class="tabbed-other">
    <input type="image" border="0" name="{@id}" src="{$context}/wcf/tabbed/other.png" onclick="pageAlert=false"/>
    <xsl:text> </xsl:text>
    <xsl:value-of select="@label"/>
  </td>
</xsl:template>
