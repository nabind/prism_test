...
public class EduCenterTOMapper implements RowMapper<EduCenterTO> {
	...
	private static final IAppLogger logger = 
			LogFactory.getLoggerInstance(EduCenterTOMapper.class.getName());
	...
	public EduCenterTO mapRow(ResultSet rs, int rowNum) throws SQLException {
		...
		try{
			...
			if(eduCenterTO.getUserId() != 0){
				List<Map<String, Object>> roleListDb = jdbcTemplate.queryForList(
						IQueryConstants.GET_EDUCENTER_USER_ROLE, eduCenterTO.getUserId());
				if (roleListDb.size() > 0){
					...
					//eduCenterTO.setAvailableRoleList(RoleTOs);
				}		
			}
