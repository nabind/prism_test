...
public class CustomReportServlet extends AbstractServlet
{
		...
		) throws IOException, ServletException
	{
		...
		try
		{
			...
			else
			{
				...
				runReport(request, webReportContext, response);
				/** end PRISM **/
	...
	public void runReport(
		...
		WebReportContext webReportContext/** PRISM **/ , HttpServletResponse response /** end PRISM **/
...
//		JRXhtmlExporter exporter = new JRXhtmlExporter(getJasperReportsContext());
		...
		JRXhtmlExporter exporter = new JRXhtmlExporter();  // Add custom
		...
		exporter.setParameter(JRExporterParameter.JASPER_PRINT, jasperPrintAccessor.getJasperPrint());
		exporter.setParameter(JRExporterParameter.OUTPUT_WRITER, writer);
		String printSession = net.sf.jasperreports.j2ee.servlets.ImageServlet.DEFAULT_JASPER_PRINT_SESSION_ATTRIBUTE + request.getParameter(WebUtil.REQUEST_PARAMETER_REPORT_URI);
