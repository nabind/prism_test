		...
		if (list.length > 0 && replaced)
		{
			...
			replaced.find('option, optgroup').each(function(i)
			{
				...
				if(customCount == 0 && select.hasClass('check-list')) {
					$('<'+node+((classes.length > 0) ? ' class="all-option '+classes.join(' ')+'"' : '')+' onClick="selectAllOption(event, '+$(this).attr('tgt')+');"><b class="tag compact icon icon-right with-tooltip" title="Click to Select/Deselect All options" style="padding: 5px 10px;"><span class="list-count" style="display:inline; position: relative; margin-left:8px">Select / Deselect All</span></b></'+node+'>')
						.appendTo(list)
