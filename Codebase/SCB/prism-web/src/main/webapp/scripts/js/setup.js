...
;(function($, window, document, undefined)
{
	 ...
	 */
	/* Comminting out for PRISM to make it faster in IE */
	/*
	yepnope({
		test : Modernizr.mq('(min-width:0)'),
		nope : ['scripts/js/libs/respond.min.js']
	});*/
	...
	{
		// Test
		if (fixedTest.offset() != null && fixedTest.offset().top != doc.scrollTop())
		{
			// Define
			supportFixed = false;
	...
	$.template.init = function()
	{
		...
		if ($.fn.customScroll)
		{
			...
			// Commenting for pridm as not required scroll for menu
			// First call
			//updateMenuScroll();
