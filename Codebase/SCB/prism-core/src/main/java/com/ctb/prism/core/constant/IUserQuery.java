...
public interface IUserQuery {
	...
	*/
	...
	public static final String GET_USER_DETAILS_ON_FIRST_LOAD = CustomStringUtil.appendString(
			...
			"	FROM (SELECT	",
			"	  CAST(USR.USERID AS varchar(18)) AS USERROWID,	",
			"	  USR.USERID,	",
			"	  USR.USERNAME,	",
			"	  ISNULL(USR.LAST_NAME, '') + ' ' + ISNULL(USR.FIRST_NAME, '') AS FULLNAME,	",
	...
	.appendString("	WITH h$cte	",
			...
			"	AND orguser.ORG_NODEID = OLP.ORG_NODEID	",
			"	AND OLP.CUST_PROD_ID = ?	",
			"	AND UPPER(USR.USERNAME) > UPPER(?)) ABC	",
			"	ORDER BY UPPER(ABC.USERNAME)	");
	...
	.appendString("	WITH h$cte	",
			...
			"	OR UPPER(USR.FIRST_NAME) LIKE UPPER(?))) ABC	",
			"	ORDER BY UPPER(ABC.USERNAME)	");
	...
	public static final String GET_USER_DATA = CustomStringUtil.appendString(
			...
			"	  USR.USERNAME,	",
			"	  ISNULL(USR.LAST_NAME, '') + ' ' + ISNULL(USR.FIRST_NAME, '') AS FULLNAME,	",
			"	  CASE USR.ACTIVATION_STATUS	",
			"	    WHEN 'AC' THEN 'ENABLED'	",
			"	    ELSE 'DISABLED'	",
			"	  END AS STATUS,	",
			"	  HIER.ORG_NODE_NAME,	",
			"	  HIER.ORG_NODEID,	",
			...
			"	         '/' + ORG_LABEL	",
			"	       FROM TEMP	",
			"	       WHERE ORG_LEVEL = a.ORG_LEVEL	",
			"	       FOR xml PATH (''), TYPE)	",
			"	       .value('.', 'VARCHAR(MAX)'), 1, 1, '') AS ORG_LABEL	",
			"	     FROM TEMP a	",
			"	     GROUP BY a.ORG_LEVEL) OTS	",
			"	WHERE ORGUSER.USERID = USR.USERID	",
			"	AND HIER.ORG_NODEID = ORGUSER.ORG_NODEID	",
			"	AND ORGUSER.ORG_NODE_LEVEL <> 0	",
			"	AND USR.ACTIVATION_STATUS != 'SS'	",
			...
			"	AND ORGUSER.ORG_NODEID = OLP.ORG_NODEID	",
			"	AND OLP.CUST_PROD_ID = ?	",
			"	AND HIER.ORG_NODE_LEVEL = OTS.ORG_LEVEL) ABC	",
			"	LEFT JOIN (SELECT	",
			"	  USERID,	",
			"	  STUFF((SELECT	",
			"	    ' ,' + DESCRIPTION	",
			...
			"	  FOR xml PATH (''), TYPE)	",
			"	  .value('.', 'VARCHAR(MAX)'), 1, 2, '') AS DESCRIPTION	",
			"	FROM RL a	",
			"	GROUP BY USERID) R	",
			"	  ON ABC.USERID = R.USERID) b	",
