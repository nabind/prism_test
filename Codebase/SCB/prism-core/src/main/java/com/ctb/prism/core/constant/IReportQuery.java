...
public interface IReportQuery {
	...
	public static final String GET_INPUT_CONTROL_DETAILS = CustomStringUtil.appendString(
			"SELECT RE.LABEL LBL, RUIC.INPUT_CONTROL_ID CONTROLID, RE.NAME LABELID, RUIC.CONTROL_INDEX SEQ, ",
			...
			" WHERE REPORT_UNIT_ID = ",
				"(SELECT RU.ID FROM JIREPORTUNIT RU, JIRESOURCE R, JIRESOURCEFOLDER RF ",
				" WHERE RU.MAINREPORT = R.ID AND R.PARENT_FOLDER = RF.ID AND RF.URI = ? ) ",
			" AND DT.id(+) = IC.DATA_TYPE ",
			" AND RE.ID = RUIC.INPUT_CONTROL_ID ",
			" AND RUIC.INPUT_CONTROL_ID = IC.ID ORDER BY SEQ");
