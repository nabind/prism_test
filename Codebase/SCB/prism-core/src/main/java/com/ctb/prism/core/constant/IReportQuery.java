...
public interface IReportQuery {
	...
	public static final String GET_INPUT_CONTROL_DETAILS = CustomStringUtil.appendString(			
			...
			"	  dbo.F_GET_LIST_OF_VALUES(IC.LIST_OF_VALUES) LIST_OF_VALUES,	",
			"	  DT.type FIELD_TYPE,	",
			...
			"	     JIRESOURCE RE,	",
			"	     jidatatype DT	",
			"	     RIGHT OUTER JOIN JIINPUTCONTROL IC	",
			"	       ON DT.id = IC.DATA_TYPE	",
			"	WHERE REPORT_UNIT_ID = (SELECT	",
			"	  RU.ID	",
			...
			"	AND RUIC.INPUT_CONTROL_ID = IC.ID	",
			"	ORDER BY SEQ");
