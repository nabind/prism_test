CREATE OR REPLACE FUNCTION SF_GET_CLASS_GRP_DWNLD(LoggedInUserJasperOrgId IN ORG_NODE_DIM.ORG_NODEID%TYPE,
  ...
  ************************************************************************************/
  ...
  CURSOR c_Get_Class_Default(p_ProductId   PRODUCT.PRODUCTID%TYPE,
                             p_SchoolId    ORG_NODE_DIM.ORG_NODEID%TYPE,
                             test_program1 NUMBER,
                             p_Gradeid     GRADE_DIM.GRADEID%TYPE) IS
    SELECT OND.ORG_NODEID, OND.ORG_NODE_NAME
     ...
     WHERE CPL.PRODUCTID = p_ProductId
       ...
       AND ((v_OrgNodeLevel <> 4 AND OND.PARENT_ORG_NODEID = p_SchoolId) OR
           (v_OrgNodeLevel = 4 AND
           OND.ORG_NODEID = LoggedInUserJasperOrgId))
       AND OND.ORG_NODE_LEVEL = 4
     ORDER BY OND.ORG_NODE_NAME;

  CURSOR c_Get_Class_Cascade(test_program4 NUMBER) IS
    SELECT OND.ORG_NODEID, OND.ORG_NODE_NAME
      FROM CUST_PRODUCT_LINK      CPL,
           ORG_PRODUCT_LINK       ORPD,
           ORG_NODE_DIM           OND,
     ...
     WHERE CPL.PRODUCTID = p_test_administration
       ...
       AND (p_grade = -1 OR GSL.GRADEID = p_grade)
       AND ((v_OrgNodeLevel <> 4 AND OND.PARENT_ORG_NODEID = p_school) OR
           (v_OrgNodeLevel = 4 AND
           OND.ORG_NODEID = LoggedInUserJasperOrgId))
       AND OND.ORG_NODE_LEVEL = 4
     ORDER BY 2;

  CURSOR c_Get_Class_For_Level4(p_Product_Id PRODUCT.PRODUCTID%TYPE,
                                test_program NUMBER,
                                p_Grade_id   GRADE_DIM.GRADEID%TYPE) IS
    SELECT OND.ORG_NODEID, OND.ORG_NODE_NAME
      FROM CUST_PRODUCT_LINK      CPL,
           ORG_PRODUCT_LINK       ORPD,
           ORG_NODE_DIM           OND,
     ...
     WHERE CPL.PRODUCTID = p_Product_Id
       ...
       AND ((v_OrgNodeLevel = 4 AND
           OND.ORG_NODEID = LoggedInUserJasperOrgId))
       AND OND.ORG_NODE_LEVEL = 4
     ORDER BY 2;

  CURSOR c_Get_Corp_Default(p_ProductId   PRODUCT.PRODUCTID%TYPE,
                            test_program2 NUMBER) IS
    SELECT A.ORG_NODEID
  ...
  CURSOR c_Get_School_Default(p_ProductId   PRODUCT.PRODUCTID%TYPE,
                              p_CorpId      ORG_NODE_DIM.ORG_NODEID%TYPE,
                              test_program3 NUMBER) IS
    SELECT A.ORG_NODEID
      FROM (SELECT OND.ORG_NODEID, OND.ORG_NODE_NAME
              FROM CUST_PRODUCT_LINK CPL,
                   ORG_PRODUCT_LINK  ORPD,
  ...
  CURSOR c_Get_Grade(Product_Id PRODUCT.PRODUCTID%TYPE,
  ...
  CURSOR c_Get_Grade_For_ClassPrfGrpng(Product_Id PRODUCT.PRODUCTID%TYPE,
                                       org_id     ORG_NODE_DIM.ORG_NODEID%TYPE) IS
    SELECT A.GRADEID
      FROM (SELECT DISTINCT GRD.GRADEID, GRD.GRADE_NAME, GRD.GRADE_SEQ
              FROM GRADE_SELECTION_LOOKUP GSL,
  ...
  CURSOR c_Get_Product_Default IS(
    SELECT A.PRODUCTID
      FROM (SELECT PDT.PRODUCTID, PDT.PRODUCT_NAME, PDT.PRODUCT_SEQ
            ...
            --AND PDT.PRODUCT_CODE <> 'ISTEPS15' ---COMMENTED FOR GROUP DOWNLOADS
...
BEGIN
  ...
  IF v_OrgNodeLevel = 4 THEN
    ...
    IF p_type = 'CLASS_PRF_GRPNG' THEN
      ---for class proficiency grouping
      FOR r_Get_Grade_For_ClassPrfGrpng IN c_Get_Grade_For_ClassPrfGrpng(v_ProductId,
                                                                         LoggedInUserJasperOrgId) LOOP
    ...
    FOR r_Get_Class_For_Level4 IN c_Get_Class_For_Level4(v_ProductId,
                                                         v_test_program,
                                                         v_GradeId) LOOP
      t_PRS_PGT_GLOBAL_TEMP_OBJ := PRS_PGT_GLOBAL_TEMP_OBJ();
  ...
  ELSIF /* p_test_administration = -99   OR  p_test_administration IS NULL
   ...
   (p_grade = -99 OR p_grade IS NULL) THEN
    ...
    ---get default Class
    FOR r_Get_Class_Default IN c_Get_Class_Default(v_ProductId,
                                                   v_SchoolId,
                                                   v_test_program,
                                                   v_GradeId) LOOP
      t_PRS_PGT_GLOBAL_TEMP_OBJ := PRS_PGT_GLOBAL_TEMP_OBJ();
  ...
  ---without changing input controls and clicking on Refresh
   p_school = -99 AND
   (p_grade <> -99 OR p_grade IS NOT NULL OR p_grade <> -1 OR p_grade <> -2) THEN
    ---get default Corp
    FOR r_Get_Corp_Default IN c_Get_Corp_Default(p_test_administration,
                                                 v_test_program) LOOP
      v_CorpId := r_Get_Corp_Default.ORG_NODEID;
    END LOOP;
  
    ---get default School
    FOR r_Get_School_Default IN c_Get_School_Default(p_test_administration,
                                                     v_CorpId,
                                                     v_test_program) LOOP
      v_SchoolId := r_Get_School_Default.ORG_NODEID;
    END LOOP;
    ...
    ---get default Class
    FOR r_Get_Class_Default IN c_Get_Class_Default(p_test_administration,
                                                   v_SchoolId,
                                                   v_test_program,
                                                   p_grade) LOOP
      t_PRS_PGT_GLOBAL_TEMP_OBJ := PRS_PGT_GLOBAL_TEMP_OBJ();
    ...
    END LOOP;
  ELSE
    FOR r_Get_Class_Cascade IN c_Get_Class_Cascade(v_test_program) LOOP
      t_PRS_PGT_GLOBAL_TEMP_OBJ := PRS_PGT_GLOBAL_TEMP_OBJ();
  ...
  IF p_type = 'ADD_ALL' AND t_PRS_COLL_PGT_GLOBAL_TEMP_OBJ.COUNT >= 1 AND
     v_OrgNodeLevel <> 4 THEN
    t_PRS_PGT_GLOBAL_TEMP_OBJ     := PRS_PGT_GLOBAL_TEMP_OBJ();
