CREATE OR REPLACE FUNCTION SF_GET_CORP_ISTEPS15 (
...
PRAGMA AUTONOMOUS_TRANSACTION;
 ...
 CURSOR c_Get_Corp_Default (p_ProductId PRODUCT.PRODUCTID%TYPE,test_program1 NUMBER)
  IS
...
CURSOR c_Get_Corp_Cascade (test_program NUMBER)
  IS
  SELECT  OND.ORG_NODEID, OND.ORG_NODE_NAME
  FROM CUST_PRODUCT_LINK     CPL,
       ORG_PRODUCT_LINK      ORPD,
...
BEGIN
      FOR r_Get_Org_Node_Level IN c_Get_Org_Node_Level
       ...
       END IF;
         ...
         IF p_test_administration = -99   OR  p_test_administration IS NULL  THEN
          ...
          ELSE
           FOR r_Get_Corp_Cascade IN c_Get_Corp_Cascade (v_test_program)
            LOOP
                     t_PRS_PGT_GLOBAL_TEMP_OBJ := PRS_PGT_GLOBAL_TEMP_OBJ();
      ...
      IF p_type = 'ADD_ALL' AND t_PRS_COLL_PGT_GLOBAL_TEMP_OBJ.COUNT >=1  THEN
            ...
            t_PRS_PGT_GLOBAL_TEMP_OBJ.vc2 := 'All Corps/Dioceses';
            t_PRS_PGT_GLOBAL_TEMP_OBJ.vc3 := -1;

            t_PRS_COLL_PGT_GLOBAL_TEMP_OBJ.EXTEND(1);
            t_PRS_COLL_PGT_GLOBAL_TEMP_OBJ(t_PRS_COLL_PGT_GLOBAL_TEMP_OBJ.COUNT):= t_PRS_PGT_GLOBAL_TEMP_OBJ;

      ELSIF p_type = 'ADD_ALL_FOR_LVL1' AND t_PRS_COLL_PGT_GLOBAL_TEMP_OBJ.COUNT >=1 AND v_OrgNodeLevel =1  THEN
            t_PRS_PGT_GLOBAL_TEMP_OBJ := PRS_PGT_GLOBAL_TEMP_OBJ();
            t_PRS_PGT_GLOBAL_TEMP_OBJ.vc1 := -1;
            t_PRS_PGT_GLOBAL_TEMP_OBJ.vc2 := 'All Corps/Dioceses';
            t_PRS_PGT_GLOBAL_TEMP_OBJ.vc3 := -1;
