CREATE OR REPLACE PROCEDURE SP_STG_MO_HIER_EXTRACT_VAL(IN_PROCESS_ID IN NUMBER,
...
BEGIN
    ...
    SELECT COUNT(*)
      ...
      FROM (SELECT PROCESS_ID, LISTAGG(ERRM, ';') WITHIN
             ...
             ORDER BY ERRM) AS ERRM
              FROM (SELECT DISTINCT F.PROCESS_ID,
                                    CASE
                                      WHEN F.ELEMENT_A_HIERARCHY_CODE IS NULL THEN
                                       'ELEMENT_A_ORG_CODE IS NULL'
                                      WHEN F.ELEMENT_A_STRUCTURE_ELEMENT IS NULL THEN
                                       'ELEMENT_A_STRUCTURE_ELEMENT IS NULL'
                                      WHEN F.ELEMENT_A_STRUCTURE_LEVEL IS NULL THEN
                                       'ELEMENT_A_LEVEL IS NULL'
                                    END AS ERRM
                      FROM STG_MO_HIER_EXTRACT F
                     WHERE (F.ELEMENT_A_HIERARCHY_CODE IS NULL OR
                           F.ELEMENT_A_STRUCTURE_ELEMENT IS NULL OR
                           F.ELEMENT_A_STRUCTURE_LEVEL IS NULL)
                       AND 1 <= (SELECT MAX(ORG_LEVEL)
                                   FROM ORG_TP_STRUCTURE A,
                                        (SELECT DISTINCT TP_ID
                                           FROM TEST_PROGRAM          T,
                                                STG_MO_HIER_EXTRACT H
                    ...
                    SELECT DISTINCT F.PROCESS_ID,
                                    CASE
                                      WHEN F.ELEMENT_B_HIERARCHY_CODE IS NULL THEN
                                       'ELEMENT_B_ORG_CODE IS NULL'
                                      WHEN F.ELEMENT_B_STRUCTURE_ELEMENT IS NULL THEN
                                       'ELEMENT_B_STRUCTURE_ELEMENT IS NULL'
                                      WHEN F.ELEMENT_B_STRUCTURE_LEVEL IS NULL THEN
                                       'ELEMENT_B_LEVEL IS NULL'
                                    END AS ERRM
                      FROM STG_MO_HIER_EXTRACT F
                     WHERE (F.ELEMENT_B_HIERARCHY_CODE IS NULL OR
                           F.ELEMENT_B_STRUCTURE_ELEMENT IS NULL OR
                    ...
                    SELECT DISTINCT F.PROCESS_ID,
                                    CASE
                                      WHEN F.ELEMENT_C_HIERARCHY_CODE IS NULL THEN
                                       'ELEMENT_C_ORG_CODE IS NULL'
                                      WHEN F.ELEMENT_C_STRUCTURE_ELEMENT IS NULL THEN
                                       'ELEMENT_C_STRUCTURE_ELEMENT IS NULL'
                                      WHEN F.ELEMENT_C_STRUCTURE_LEVEL IS NULL THEN
                                       'ELEMENT_C_LEVEL IS NULL'

                                    END AS ERRM
                      FROM STG_MO_HIER_EXTRACT F
                     WHERE (F.ELEMENT_C_HIERARCHY_CODE IS NULL OR
                           F.ELEMENT_C_STRUCTURE_ELEMENT IS NULL OR
                   ...
                   /* UNION ALL
                    ...
                    SELECT DISTINCT F.PROCESS_ID,
                                    CASE
                                      ...
                                      WHEN F.ELEMENT_D_STRUCTURE_ELEMENT IS NULL THEN
                     ...
                     WHERE (F.ELEMENT_D_ORG_NODE_CODE IS NULL OR
                           F.ELEMENT_D_STRUCTURE_ELEMENT IS NULL OR
                    ...
                    SELECT DISTINCT F.PROCESS_ID,
                                    CASE
                                      ...
                                      WHEN F.ELEMENT_E_STRUCTURE_ELEMENT IS NULL THEN
                     ...
                     WHERE (F.ELEMENT_E_ORG_NODE_CODE IS NULL OR
                           F.ELEMENT_E_STRUCTURE_ELEMENT IS NULL OR
                    ...
                    SELECT DISTINCT F.PROCESS_ID,
                                    CASE
                                      ...
                                      WHEN F.ELEMENT_F_STRUCTURE_ELEMENT IS NULL THEN
                     ...
                     WHERE (F.ELEMENT_F_ORG_NODE_CODE IS NULL OR
                           F.ELEMENT_F_STRUCTURE_ELEMENT IS NULL OR
                    ...
                    SELECT DISTINCT F.PROCESS_ID,
                                    CASE
                                      ...
                                      WHEN F.ELEMENT_G_STRUCTURE_ELEMENT IS NULL THEN
                     ...
                     WHERE (F.ELEMENT_G_ORG_NODE_CODE IS NULL OR
                           F.ELEMENT_G_STRUCTURE_ELEMENT IS NULL OR
               ...
               WHERE (F.ELEMENT_A_HIERARCHY_CODE IS NULL OR
                 ...
                 AND 1 <=
                     (SELECT MAX(ORG_LEVEL)
                        FROM ORG_TP_STRUCTURE A,
                             (SELECT DISTINCT TP_ID
                                FROM TEST_PROGRAM T, STG_MO_HIER_EXTRACT H
                               WHERE T.TP_CODE = H.ORG_TP) B
                       WHERE A.TP_ID = B.TP_ID)
      ...
      FROM (SELECT PROCESS_ID, LISTAGG(ERRM, ' :: ') WITHIN
             ...
             ORDER BY ERRM) AS ERRM
              FROM (SELECT F.PROCESS_ID,
                           F.ELEMENT_A_HIERARCHY_CODE || ',' ||
                           F.ELEMENT_B_HIERARCHY_CODE || ',' ||
                           F.ELEMENT_C_HIERARCHY_CODE/* || ',' ||
                    ...
                    SELECT F.PROCESS_ID,
                           F.ELEMENT_A_HIERARCHY_CODE || ',' ||
                           F.ELEMENT_B_HIERARCHY_CODE || ',' ||
                           F.ELEMENT_C_HIERARCHY_CODE AS LIST_OF_ORGS
                      FROM (SELECT F.PROCESS_ID,
                                   F.ELEMENT_A_HIERARCHY_CODE,
                                   F.ELEMENT_A_STRUCTURE_LEVEL,
                                   F.ELEMENT_B_HIERARCHY_CODE,
                    ...
                    SELECT F.PROCESS_ID,
                           F.ELEMENT_A_HIERARCHY_CODE || ',' ||
                           F.ELEMENT_B_HIERARCHY_CODE AS LIST_OF_ORGS
                      FROM (SELECT F.PROCESS_ID,
                                   F.ELEMENT_A_HIERARCHY_CODE,
                                   F.ELEMENT_A_STRUCTURE_LEVEL,
                                   F.ELEMENT_B_HIERARCHY_CODE,
 ...
 /* \*INDIVIDUAL HIER DUPLICATE VALIDATION START*\
  IF (V_FLAG IS NULL) THEN
    ...
    SELECT COUNT(*)
      INTO V_COUNT
      FROM (SELECT PROCESS_ID, LISTAGG(ORG_CODE, ',') WITHIN
             GROUP(
             ORDER BY ORG_CODE) AS ERRM
              FROM (SELECT F.PROCESS_ID                AS PROCESS_ID,
                           F.ELEMENT_A_ORG_NODE_CODE   AS ORG_CODE,
                           F.ELEMENT_A_STRUCTURE_LEVEL AS LEVELID
                      FROM STG_MO_HIER_EXTRACT F
                     WHERE 1 <= (SELECT MAX(ORG_LEVEL)
                                   FROM ORG_TP_STRUCTURE A,
                                        (SELECT DISTINCT TP_ID
                                           FROM TEST_PROGRAM          T,
                                                STG_MO_HIER_EXTRACT H
                     ...
                     GROUP BY F.PROCESS_ID,
                              ...
                              F.ELEMENT_A_STRUCTURE_LEVEL
                    HAVING COUNT(DISTINCT F.ELEMENT_A_HIERARCHY_NAME) > 1

                    UNION ALL
                    SELECT F.PROCESS_ID                AS PROCESS_ID,
                           F.ELEMENT_B_ORG_NODE_CODE   AS ORG_CODE,
                           F.ELEMENT_B_STRUCTURE_LEVEL AS LEVELID
                      FROM STG_MO_HIER_EXTRACT F
                     WHERE 2 <= (SELECT MAX(ORG_LEVEL)
                                   FROM ORG_TP_STRUCTURE A,
                                        (SELECT DISTINCT TP_ID
                                           FROM TEST_PROGRAM          T,
                                                STG_MO_HIER_EXTRACT H
                     ...
                     GROUP BY F.PROCESS_ID,
                              ...
                              F.ELEMENT_B_STRUCTURE_LEVEL
                    HAVING COUNT(DISTINCT F.ELEMENT_B_HIERARCHY_NAME) > 1
                    UNION ALL
                    SELECT F.PROCESS_ID                AS PROCESS_ID,
                           F.ELEMENT_C_ORG_NODE_CODE   AS ORG_CODE,
                           F.ELEMENT_C_STRUCTURE_LEVEL AS LEVELID
                      FROM STG_MO_HIER_EXTRACT F
                     WHERE 3 <= (SELECT MAX(ORG_LEVEL)
                                   FROM ORG_TP_STRUCTURE A,
                                        (SELECT DISTINCT TP_ID
                                           FROM TEST_PROGRAM          T,
                                                STG_MO_HIER_EXTRACT H
                     ...
                     GROUP BY F.PROCESS_ID,
                              ...
                              F.ELEMENT_C_STRUCTURE_LEVEL
                    HAVING COUNT(DISTINCT F.ELEMENT_C_HIERARCHY_NAME) > 1
                    UNION ALL
                    SELECT F.PROCESS_ID                AS PROCESS_ID,
                           F.ELEMENT_D_ORG_NODE_CODE   AS ORG_CODE,
                           F.ELEMENT_D_STRUCTURE_LEVEL AS LEVELID
                      FROM STG_MO_HIER_EXTRACT F
                     WHERE 4 <= (SELECT MAX(ORG_LEVEL)
                                   FROM ORG_TP_STRUCTURE A,
                                        (SELECT DISTINCT TP_ID
                                           FROM TEST_PROGRAM          T,
                                                STG_MO_HIER_EXTRACT H
                     ...
                     GROUP BY F.PROCESS_ID,
                              F.ELEMENT_D_ORG_NODE_CODE,
                              F.ELEMENT_D_STRUCTURE_LEVEL
                    HAVING COUNT(DISTINCT F.ELEMENT_D_HIERARCHY_NAME) > 1
                    UNION ALL
                    SELECT F.PROCESS_ID                AS PROCESS_ID,
                           F.ELEMENT_E_ORG_NODE_CODE   AS ORG_CODE,
                           F.ELEMENT_E_STRUCTURE_LEVEL AS LEVELID
                      FROM STG_MO_HIER_EXTRACT F
                     WHERE 5 <= (SELECT MAX(ORG_LEVEL)
                                   FROM ORG_TP_STRUCTURE A,
                                        (SELECT DISTINCT TP_ID
                                           FROM TEST_PROGRAM          T,
                                                STG_MO_HIER_EXTRACT H
                     ...
                     GROUP BY F.PROCESS_ID,
                              F.ELEMENT_E_ORG_NODE_CODE,
                              F.ELEMENT_E_STRUCTURE_LEVEL
                    HAVING COUNT(DISTINCT F.ELEMENT_E_HIERARCHY_NAME) > 1
                    UNION ALL
                    SELECT F.PROCESS_ID                AS PROCESS_ID,
                           F.ELEMENT_F_ORG_NODE_CODE   AS ORG_CODE,
                           F.ELEMENT_F_STRUCTURE_LEVEL AS LEVELID
                      FROM STG_MO_HIER_EXTRACT F
                     WHERE 6 <= (SELECT MAX(ORG_LEVEL)
                                   FROM ORG_TP_STRUCTURE A,
                                        (SELECT DISTINCT TP_ID
                                           FROM TEST_PROGRAM          T,
                                                STG_MO_HIER_EXTRACT H
                     ...
                     GROUP BY F.PROCESS_ID,
                              F.ELEMENT_F_ORG_NODE_CODE,
                              F.ELEMENT_F_STRUCTURE_LEVEL
                    HAVING COUNT(DISTINCT F.ELEMENT_F_HIERARCHY_NAME) > 1
                    UNION ALL
                    SELECT F.PROCESS_ID                AS PROCESS_ID,
                           F.ELEMENT_G_ORG_NODE_CODE   AS ORG_CODE,
                           F.ELEMENT_G_STRUCTURE_LEVEL AS LEVELID
                      FROM STG_MO_HIER_EXTRACT F
                     WHERE 7 <= (SELECT MAX(ORG_LEVEL)
                                   FROM ORG_TP_STRUCTURE A,
                                        (SELECT DISTINCT TP_ID
                                           FROM TEST_PROGRAM          T,
                                                STG_MO_HIER_EXTRACT H
                     ...
                     GROUP BY F.PROCESS_ID,
                              F.ELEMENT_G_ORG_NODE_CODE,
                              F.ELEMENT_G_STRUCTURE_LEVEL
                    HAVING COUNT(DISTINCT F.ELEMENT_G_HIERARCHY_NAME) > 1)
             GROUP BY PROCESS_ID);

    IF (V_COUNT > 0) THEN
      SELECT PROCESS_ID, LISTAGG(ORG_CODE, ',') WITHIN
       GROUP(
       ORDER BY ORG_CODE) AS ERRM
        INTO V_ERR_PROCESS_ID, V_ERRM
        FROM (SELECT F.PROCESS_ID                AS PROCESS_ID,
      ...
      IF (V_ERR_PROCESS_ID IS NOT NULL) THEN
        V_FLAG := 'Y';
        V_ERRM := '[' || TO_CHAR(SYSDATE, 'HH:MI:SS PM') || ']' ||
                  ' SAME SPECIAL_CODE AND LEVEL WITH DIFFERENT ORG_NAME IN FILE ISSUE. The ORG_NODE_CODE with issues are := ' ||
                  V_ERRM;
        V_LOG  := V_LOG || CHR(10) || V_ERRM;
      END IF;
    END IF;
  END IF;
  \*INDIVIDUAL HIER DUPLICATE VALIDATION END*\*/
...
END SP_STG_MO_HIER_EXTRACT_VAL;
/
