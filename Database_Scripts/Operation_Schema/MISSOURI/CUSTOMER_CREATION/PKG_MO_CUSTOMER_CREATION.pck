  ...
  PROCEDURE SP_MO_CREATE_CUSTOMER_NEW(P_CUSTOMER_NAME  IN VARCHAR2,
    ...
    CURSOR C_DEMO IS
      SELECT DEMOID,
             ...
             CATEGORY,
             IS_DEMO_VALUE_AVL
        FROM MISSOURI.DEMOGRAPHIC
       WHERE CUSTOMERID = V_OLD_CUSTOMERID;
  
  BEGIN
  
    IF P_PP_TPCODE NOT IN ('MAPSMTSPR2015', 'MAPINTSUM2015','MAPSUMTSPR2016') THEN
    ...
    INSERT INTO MISSOURI.CUTSCORESCALESCORE
      ...
      SELECT CUTSCORESCALESCOREID,
             ...
             SYSDATE,
             SYSDATE
        FROM MISSOURI.CUTSCORESCALESCORE
       WHERE CUST_PROD_ID = V_OLD_CUST_PRODID;
  
    INSERT INTO MISSOURI.ARTICLE_METADATA
      (ARTICLEID,
       ARTICLE_NAME,
       CUST_PROD_ID,
       SUBT_OBJ_MAPID,
    ...
    IF V_TP_COUNT = 0 THEN
      ...
      /* SELECT NVL(COUNT(1),0) 
       INTO V_PP_TPCODE_CHK 
       FROM PRISMGLOBAL.TEST_PROGRAM 
      WHERE TP_CODE = P_PP_TPCODE; */
    
      /*IF V_PP_TPCODE_CHK > 0 THEN
      RAISE EXP_TPCODE_CHK;
      END IF;*/
    ...
    SELECT MAX(DEMOID) INTO V_DEMOID FROM MISSOURI.DEMOGRAPHIC;
    ...
    FOR R_DEMO IN C_DEMO LOOP
      ...
      INSERT INTO MISSOURI.DEMOGRAPHIC
        (DEMOID,
         DEMO_NAME,
         DEMO_CODE,
         DEMO_MODE,
      ...
      INSERT INTO MISSOURI.DEMOGRAPHIC_VALUES
        (DEMO_VALID,
         DEMOID,
         DEMO_VALUE_NAME,
         DEMO_VALUE_CODE,
    ...
    IF V_ORG_NODE_ID IS NULL THEN
      ...
      INSERT INTO MISSOURI.ORG_NODE_DIM
        (ORG_NODEID,
         ORG_NODE_NAME,
         ORG_NODE_CODE,
         ORG_NODE_LEVEL,
    ...
    MISSOURI.PRC_MV_REFRESH_SCORE_RANGE();
