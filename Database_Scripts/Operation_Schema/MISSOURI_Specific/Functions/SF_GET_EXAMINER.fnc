CREATE OR REPLACE FUNCTION SF_GET_EXAMINER(LOGGEDINUSERJASPERORGID IN ORG_NODE_DIM.ORG_NODEID%TYPE,
  ...
  ************************************************************************************/
  ...
  CURSOR C_GET_EXAMINER(P_CUST_PRODUCT_ID_4 CUST_PRODUCT_LINK.CUST_PROD_ID%TYPE,
                        ORG_ID              ORG_NODE_DIM.ORG_NODEID%TYPE,
                        P_GRADEID           GRADE_DIM.GRADEID%TYPE,
                        P_SUBTESTID         SUBTEST_DIM.SUBTESTID%TYPE) IS
    SELECT DISTINCT ED.EXAMINERID,
                    ED.LAST_NAME || ', ' || ED.FIRST_NAME EXAMINER_NAME
     ...
     WHERE ED.EXAMINERID = OEL.EXAMINERID
       ...
       AND OEL.ORG_NODEID = ORG_ID
       AND OEL.GRADEID = P_GRADE
       AND OEL.SUBTESTID = P_SUBTESTID
