CREATE OR REPLACE PACKAGE PKG_ADMIN_MODULE IS
  ...
  PROCEDURE SP_GET_USER_DETAILS(P_IN_USERNAME          IN USERS.USERNAME%TYPE,
                                ...
                                V_OUT_LAST_PASSWORD_DATE OUT DATE,
                                P_OUT_EXCEP_ERR_MSG    OUT VARCHAR2);
...
CREATE OR REPLACE PACKAGE BODY PKG_ADMIN_MODULE IS
  ...
  CURSOR c_chk_user(P_IN_USERNAME IN USERS.USERNAME%TYPE) IS
  SELECT COUNT(1)
     -- INTO INCOUNT
      FROM USERS
     WHERE UPPER(USERNAME) = UPPER(P_IN_USERNAME);

  CURSOR c_get_roleid(P_IN_ROLES IN ROLE.ROLE_NAME%TYPE) IS
   SELECT ROLEID
                  FROM ROLE
  ...
  rec_get_roleid c_get_roleid%ROWTYPE;
  ...
  OPEN c_chk_user(P_IN_USERNAME);
  FETCH c_chk_user INTO INCOUNT;
  CLOSE c_chk_user;
   ...
   FOR rec_get_roleid IN c_get_roleid(P_IN_ROLES)

       LOOP
        INSERT INTO USER_ROLE
          (ROLEID, USERID, CREATED_DATE_TIME)
        VALUES
          (rec_get_roleid.ROLEID, USERSEQID, SYSDATE);
  ...
  PROCEDURE SP_CREATE_PARENT(P_IN_USERNAME         IN USERS.USERNAME%TYPE,
    ...
    P_CUST_PROD_ID           INVITATION_CODE.CUST_PROD_ID%TYPE:= NULL;
    P_CUSTOMERID             CUST_PRODUCT_LINK.CUSTOMERID%TYPE := NULL;
    P_ADMIN_ID               CUST_PRODUCT_LINK.ADMINID%TYPE := NULL;
    P_ICID                   INVITATION_CODE.ICID%TYPE:= NULL;
    P_TOTAL_ATTEMPT          NUMBER:= 0;

  CURSOR c_get_inv_code_dtl(P_IN_INVITAIONCODE IN INVITATION_CODE.INVITATION_CODE%TYPE) IS
   SELECT IC.TOTAL_AVAILABLE,ORG_NODEID,CUST_PROD_ID,ICID,TOTAL_ATTEMPT
     -- INTO CLAIMAVAILABILITY,P_NODE_ID
  ...
  CURSOR c_get_customer_id (p_cust_prod_id IN CUST_PRODUCT_LINK.CUST_PROD_ID%TYPE) IS
  SELECT DISTINCT CPL.CUSTOMERID,CPL.ADMINID
              FROM CUST_PRODUCT_LINK CPL
             WHERE CPL.CUST_PROD_ID = p_cust_prod_id;

  BEGIN

    /*SELECT IC.TOTAL_AVAILABLE,ORG_NODEID
      INTO CLAIMAVAILABILITY,P_NODE_ID
      FROM INVITATION_CODE IC
     WHERE IC.INVITATION_CODE = P_IN_INVITAIONCODE
       AND IC.ACTIVATION_STATUS = 'AC'
  ...
  OPEN c_get_inv_code_dtl(P_IN_INVITAIONCODE);
  FETCH c_get_inv_code_dtl INTO CLAIMAVAILABILITY,P_NODE_ID,P_CUST_PROD_ID,P_ICID,p_TOTAL_ATTEMPT;
  CLOSE c_get_inv_code_dtl;
    ...
    OPEN c_chk_user(P_IN_USERNAME);
     ...
     CLOSE c_chk_user;
      ...
      IF INCOUNT = 0 THEN
       ...
       OPEN c_get_customer_id(P_CUST_PROD_ID);
       FETCH c_get_customer_id INTO P_CUSTOMERID,P_ADMIN_ID;
      ...
      /*  FOR I IN (SELECT ROLEID
          ...
          INSERT INTO USER_ROLE
            (ROLEID, USERID, CREATED_DATE_TIME)
          (SELECT ROLEID,USERSEQID,SYSDATE
           FROM ROLE WHERE ROLE_NAME IN ('ROLE_USER', 'ROLE_PARENT'));
        ...
        UPDATE INVITATION_CODE
           SET TOTAL_ATTEMPT     = /*(SELECT TOTAL_ATTEMPT
                                     ...
                                     WHERE INVITATION_CODE =
                                           P_IN_INVITAIONCODE
                                       AND ACTIVATION_STATUS = 'AC')*/ P_TOTAL_ATTEMPT+ 1,
               TOTAL_AVAILABLE   = /*(SELECT TOTAL_AVAILABLE
                                      FROM INVITATION_CODE
                                     WHERE INVITATION_CODE =
                                           P_IN_INVITAIONCODE
                                       AND ACTIVATION_STATUS = 'AC')*/CLAIMAVAILABILITY - 1,
  ...
  CURSOR c_get_prop_val(P_DB_PROPERTY_NAME IN DASH_CONTRACT_PROP.DB_PROPERTY_NAME%TYPE) IS
   SELECT DB_PROPERY_VALUE
      --INTO V_PASSWORD_EXPIRY
      FROM DASH_CONTRACT_PROP
     WHERE UPPER(DB_PROPERTY_NAME) = P_DB_PROPERTY_NAME;

   CURSOR c_get_last_pass_date(P_IN_USERNAME IN USERS.USERNAME%TYPE) IS
  ...
  BEGIN
    ...
    OPEN P_OUT_REF_CURSOR FOR
      ...
      SELECT A.*
        FROM (SELECT USERS.IS_FIRSTTIME_LOGIN,
                     ...
                     NVL(USERS.LAST_LOGIN_DATE,SYSDATE) LAST_LOGIN_DATE
                FROM USERS USERS,
                     ORG_USERS ORG,
                     ORG_NODE_DIM OND,
    ...
    OPEN c_get_prop_val('PASSWORD.EXPIRY');
    FETCH c_get_prop_val INTO V_PASSWORD_EXPIRY;
    CLOSE c_get_prop_val;
    ...
    /*SELECT nvl(trunc(MAX(PH.CREATED_DATE_TIME)), TRUNC(sysdate))
     ...
     WHERE UPPER(USERS.USERNAME) = UPPER(P_IN_USERNAME)
       AND USERS.USERID = PH.USERID;*/
         ...
         OPEN c_get_last_pass_date(P_IN_USERNAME);
         FETCH c_get_last_pass_date INTO V_LAST_PASSWORD_DATE;
         CLOSE c_get_last_pass_date;
         
    V_OUT_LAST_PASSWORD_DATE := V_LAST_PASSWORD_DATE;        
    ...
    IF V_OUT_PASSWORD_EXPIRED = 'FALSE' THEN
     ...
     OPEN c_get_prop_val('PASSWORD.EXPIRY.WARNING');
    FETCH c_get_prop_val INTO V_PASSWORD_WARNING;
    CLOSE c_get_prop_val;
  ...
  BEGIN
   ...
   /* SELECT COUNT(1)
     ...
     WHERE ROLE_NAME = 'ROLE_EDU_ADMIN';*/
      ...
      OPEN c_get_role_flag;
     FETCH c_get_role_flag INTO V_ROLE_FLAG;
     CLOSE c_get_role_flag;
  ...
  CURSOR c_role_count(P_IN_USERNAME IN USERS.USERNAME%TYPE) IS
  ...
  BEGIN
   ...
   /* SELECT DISTINCT COUNT(RLE.ROLE_NAME)
     ...
     WHERE USR.USERNAME = P_IN_USERNAME
      ...
      OPEN c_role_count(P_IN_USERNAME) ;
      FETCH c_role_count INTO V_ROLE_COUNT;
      CLOSE c_role_count;
