CREATE OR REPLACE PACKAGE PKG_RESCORE_REQUEST AS
  ...
  PROCEDURE SP_SUBMIT_RESCORE_REQUEST(P_IN_STUDENT_BIO_ID   IN STUDENT_BIO_DIM.STUDENT_BIO_ID%TYPE,
                                      ...
                                      P_IN_RRF_ID           RESCORE_REQUEST_FORM.RRF_ID%TYPE,
  ...
  PROCEDURE SP_GET_SUBTEST_DETAILS(P_IN_STUDENT_BIO_ID IN STUDENT_BIO_DIM.STUDENT_BIO_ID%TYPE,
...
CREATE OR REPLACE PACKAGE BODY PKG_RESCORE_REQUEST AS
      ...
      UPDATE RESCORE_REQUEST_FORM RRF
       ...
       WHERE RRF.STUDENT_BIO_ID = P_IN_STUDENT_BIO_ID
         ...
         AND RRF.ITEMSETID IN
             (SELECT RRF1.ITEMSETID
                FROM RESCORE_REQUEST_FORM RRF1, ITEMSET_DIM ISD
               WHERE RRF1.ELIGIBLE_FOR_RESCORE = 'Y'
                 AND RRF1.ITEMSETID = ISD.ITEMSETID
                 AND RRF1.STUDENT_BIO_ID = P_IN_STUDENT_BIO_ID
  ...
  BEGIN
    ...
    UPDATE RESCORE_REQUEST_FORM RRF
       SET RRF.REQUESTED_DATE    = P_IN_REQUESTED_DATE,
           RRF.UPDATED_DATE_TIME = SYSDATE,
           RRF.REQUESTED_USERID  = P_IN_USERID
     WHERE RRF.STUDENT_BIO_ID = P_IN_STUDENT_BIO_ID;
  ...
  BEGIN
    OPEN P_OUT_CUR FOR
      SELECT DISTINCT RRF.RRF_ID RRF_ID,
                      ...
                      SD.SUBTEST_CODE SUBTEST_CODE,
                      SD.SUBTEST_SEQ SUBTEST_SEQ,
                      RRF.TEST_SESSION_NUMBER SESSION_ID,
                      '0' MODEULEID,
                      RRF.ITEMSETID ITEMSETID,
                      RRF.ITEM_NUMBER ITEM_NUMBER,
                      RRF.ITEM_PART ITEM_PART,
                      RRF.ORIGINAL_SCORE ITEM_SCORE,
                      RRF.POINTS_POSSIBLE POINT_POSSIBLE,
                      RRF.ORIGINAL_PERFORMANCE_LEVEL PERFORMANCE_LEVEL,
                      NVL(RRF.IS_REQUESTED, 'N') IS_REQUESTED,
                      RRF.REQUESTED_USERID USERID,
       ...
       WHERE RRF.ORG_NODEID = OND.ORG_NODEID
         ...
         AND RRF.SUBTESTID = SD.SUBTESTID
         AND RRF.STUDENT_BIO_ID = P_IN_STUDENT_BIO_ID
         AND RRF.ORIGINAL_PERFORMANCE_LEVEL <> '*'
       ORDER BY STUDENT_FULL_NAME,
                SUBTEST_SEQ,
                SESSION_ID,
                ITEM_NUMBER,
