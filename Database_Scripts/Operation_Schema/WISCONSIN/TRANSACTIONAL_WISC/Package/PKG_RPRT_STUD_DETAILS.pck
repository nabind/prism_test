CREATE OR REPLACE PACKAGE PKG_RPRT_STUD_DETAILS IS
 ...
 FUNCTION SF_GET_DISTRICTID(P_ORG_NODEID NUMBER)RETURN NUMBER DETERMINISTIC;

 FUNCTION SF_GET_SCHOOL_NAME_CODE(P_ORG_NODEID    NUMBER,
                                  P_COLUMNTYPE    VARCHAR2) RETURN VARCHAR2 DETERMINISTIC ;
...
/* FUNCTION SF_GET_SUBTEST_DEMO_VALID(P_SUBTESTID      NUMBER,
 ...
 ---NEW VERSION FOR HOME SCHOOL                                   
 FUNCTION SF_GET_SUBTEST_DEMO_VALID( P_DEMOCODE       VARCHAR2,
                                     P_SUBTESTID      NUMBER,
                                     P_STUDENT_BIO_ID NUMBER,
                                     P_CUST_PROD_ID   IN NUMBER,
                                     P_ADMINID IN NUMBER)RETURN NUMBER DETERMINISTIC;
 ...
 FUNCTION SF_GET_SUBTEST_DEMO_EXAMINER(P_SUBTESTID       NUMBER,
  ...
  FUNCTION SF_GET_OBJECTIVE_SCORE(SUBTEST_CODE     VARCHAR2,
   ...
   FUNCTION SF_GET_OBJECTIVE_PL(SUBTEST_CODE     VARCHAR2,
                                  OBJECTIVE_CODE   VARCHAR2,
                                  P_STUDENT_BIO_ID NUMBER,
                                  P_COLUMNTYPE     VARCHAR2,
                                  P_SUBTEST_ID     NUMBER) RETURN VARCHAR DETERMINISTIC;
...
CREATE OR REPLACE PACKAGE BODY PKG_RPRT_STUD_DETAILS is
  ...
  FUNCTION SF_GET_DISTRICTID(P_ORG_NODEID NUMBER)RETURN NUMBER
    DETERMINISTIC IS
    LV_ORGID NUMBER;
   -- V_CUSTOMERID NUMBER;
  BEGIN
  ...
  -- V_CUSTOMERID := SF_GET_CUSTOMER_ID(P_CUST_PROD_ID,P_ADMINID);
   ...
   ELSIF P_COLUMNTYPE='ORG_NODE_CODE' THEN
        SELECT ORG_NODE_CODE
         ...
         WHERE ORG_NODEID = P_ORG_NODEID;
           --AND CUSTOMERID = V_CUSTOMERID;
   ELSIF P_COLUMNTYPE='ORG_MODE' THEN
        SELECT ORG_MODE
          INTO LV_ORG_NAME
          FROM ORG_NODE_DIM
         WHERE ORG_NODEID = P_ORG_NODEID;        
  ...
  BEGIN
    V_CUSTOMERID := SF_GET_CUSTOMER_ID(P_CUST_PROD_ID,P_ADMINID);
     ...
     SELECT DECODE (SUBTEST_CODE,'01','Ela','02','Math','03','Sci','Ss') INTO  V_SUBTEST_CODE
     FROM SUBTEST_DIM WHERE SUBTESTID = P_SUBTESTID;
  ...
  --COMMENTED AS ITS NOT BEEN USED IN THE MVIEW AND CREATED A NEW FUNCTION WITH THE SAME NNAME FOR HOME_SCHOOL FIELD CHANGES
  /*FUNCTION SF_GET_SUBTEST_DEMO_VALID(\*P_DEMOCODE       VARCHAR2,*\
                                     P_SUBTESTID      NUMBER,
                                     P_STUDENT_BIO_ID NUMBER,
...
--NEW VERSION OF SF_GET_SUBTEST_DEMO_VALID
FUNCTION SF_GET_SUBTEST_DEMO_VALID(  P_DEMOCODE       VARCHAR2,
                                     P_SUBTESTID      NUMBER,
                                     P_STUDENT_BIO_ID NUMBER,
                                     P_CUST_PROD_ID   IN NUMBER,
    ...
    SELECT DEMO_VALUES.DEMO_VALID
     ...
     WHERE DEMOID =
           (SELECT DEMOID
             ...
             WHERE DEMO.SUBTESTID = P_SUBTESTID --P_DEMOCODE
               AND CUSTOMERID = V_CUSTOMERID
               AND DEMO.DEMO_CODE = P_DEMOCODE||'_'||V_SUBTEST_CODE)
       AND DEMO_VALUES.STUDENT_BIO_ID = P_STUDENT_BIO_ID
       AND DEMO_VALUES.SUBTESTID =P_SUBTESTID ;
...
FUNCTION SF_GET_OBJECTIVE_PL(subtest_code     VARCHAR2,
                                  ...
                                  P_STUDENT_BIO_ID NUMBER,
                                  p_columntype     VARCHAR2,
                                  p_subtest_id     NUMBER) RETURN VARCHAR
    DETERMINISTIC IS
    v_sql          VARCHAR2(32000);
    lv_score_value  NUMBER(4,1);
    v_CUST_PROD_ID CUST_PRODUCT_LINK.CUST_PROD_ID%TYPE;
    v_pl VARCHAR2(50);
  BEGIN

    v_sql := 'SELECT ' || p_columntype || ', CUST_PROD_ID '||
             ' FROM OBJECTIVE_SCORE_FACT WHERE STUDENT_BIO_ID = :P_STUDENT_BIO_ID
            AND SUBTESTID=:p_subtest_id
            AND OBJECTIVEID IN (SELECT OBJECTIVEID FROM OBJECTIVE_DIM WHERE OBJECTIVE_CODE = :objective_CODE)';
            /*AND EXISTS (SELECT 1 FROM SUBTEST_DIM WHERE SUBTESTID =:p_subtest_id AND SUBTEST_CODE=:subtest_code)';*/
    --dbms_output.put_line(v_sql);
    EXECUTE IMMEDIATE v_sql
      INTO lv_score_value,v_CUST_PROD_ID
      USING P_STUDENT_BIO_ID, p_subtest_id, objective_CODE/*, p_subtest_id, subtest_code*/;
      
    IF p_columntype = 'PL'  THEN
     SELECT SCORE_VALUE_NAME INTO v_pl FROM SCORE_TYPE_LOOKUP WHERE CUST_PROD_ID = v_CUST_PROD_ID AND SCORE_VALUE =lv_score_value /*NVL(lv_score_value,-1)*/;
     RETURN v_pl;
    ELSE  
     RETURN v_pl;
    END IF;
