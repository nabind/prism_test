CREATE OR REPLACE FUNCTION SF_GET_DISTRICT (
...
PRAGMA AUTONOMOUS_TRANSACTION;
 ...
 CURSOR c_Get_District (p_Cust_ProductId CUST_PRODUCT_LINK.CUST_PROD_ID%TYPE,p_Dist_Type VARCHAR2)
  IS
...
BEGIN
      ...
      SELECT ORG.ORG_NODE_LEVEL INTO v_OrgNodeLevel FROM ORG_NODE_DIM ORG  WHERE ORG.ORG_NODEID = LoggedInUserJasperOrgId AND ROWNUM =1;
         ...
         IF p_test_administration = -99   OR  p_test_administration IS NULL  THEN
            ...
            FOR r_Get_Corp_Default IN c_Get_District (v_Cust_ProductId,v_District_Type)
            LOOP
                     t_PRS_PGT_GLOBAL_TEMP_OBJ := PRS_PGT_GLOBAL_TEMP_OBJ();
          ...
          ELSE
           FOR r_Get_Corp_Cascade IN c_Get_District (p_test_administration,p_district_type)
            LOOP
                     t_PRS_PGT_GLOBAL_TEMP_OBJ := PRS_PGT_GLOBAL_TEMP_OBJ();
